...
int mt_leds_parse_dt(struct mt_led_data *mdev, struct fwnode_handle *fwnode)
{
	...
	if (!ret) {
		...
		else if (!strncmp(state, "hst11", strlen("hst11")))
			mdev->conf.cdev.brightness = (mdev->conf.cdev.max_brightness / 2) * 10 / 100;
		/*Tab A11 code for AX7800W-11 by jiangyue at 20250707 end*/
		/*HS07 code for AL7761C-1 by jiangyue at 20250721 start*/
		else if (!strncmp(state, "ten-percent", strlen("ten-percent")))
