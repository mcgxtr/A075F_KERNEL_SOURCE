...
/* HS07 code for HS07-32 by gaochao at 20240228 start */
#define SECURITY_IC_ENABLE_I2C_VIA_I2C_CLK
/* HS07 code for HS07-32 by gaochao at 20240228 end */
...
/* HS07 code for HS07-32 by wangletian at 20240227 start */
static ssize_t store_k250a_node(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t len)
{
/*
SECURITY_IC_POWER_SUPPLY_VIA_LDO_CTL: ldo control, otherwise always on
SECURITY_IC_POWER_SUPPLY_VIA_GPIO: gpio control power supply
SECURITY_IC_ENABLE_I2C_VIA_I2C_CLK: CLK_IFR_I2C5 for TEE
*/
#if defined(SECURITY_IC_POWER_SUPPLY_VIA_LDO_CTL) || defined(SECURITY_IC_ENABLE_I2C_VIA_I2C_CLK) || defined(SECURITY_IC_POWER_SUPPLY_VIA_GPIO)
    int ret = 0;
#endif
    pr_info("[%s]l=%d: open flag=%d\n",
            __FUNCTION__, __LINE__, k250a_data.k250a_is_open);
    if (len <= 0) {
        pr_err("[%s]l=%d: failed to en ldo_vio18, len=%zu\n",
    ...
    if (buf[0] == '1' && k250a_data.k250a_is_open == K250A_CLOSE) {
#if defined(SECURITY_IC_POWER_SUPPLY_VIA_GPIO) // gpio control power supply
#if defined(SECURITY_IC_ENABLE_I2C_VIA_I2C_CLK) // CLK_IFR_I2C5 for TEE
        ...
        pr_err("[%s]l=%d: buf is %s, len is %zu, k250a_is_open is %d\n",
...
static int k250a_probe(struct platform_device *pdev)
{
    ...
    if (IS_ERR(k250a_data.k250a_ldo)) {
        ret = PTR_ERR(k250a_data.k250a_ldo);
        pr_err("failed to get k250a_ldo, ret is %ld\n", ret);
...
static void k250a_remove(struct platform_device *pdev)
{
    #if defined(SECURITY_IC_POWER_SUPPLY_VIA_LDO_CTL) // ldo control, otherwise always on
    int ret = 0;
    #endif
    #if defined(SECURITY_IC_POWER_SUPPLY_VIA_GPIO)
    int ret = 0;  // 为GPIO路径定义ret变量
    #endif
    pr_info("[%s]l=%d\n", __FUNCTION__, __LINE__);
...
/* HS07 code for HS07-32 by wangletian at 20240227 end */
