...
/* Tab A11_BOS code for P250902-07125 by xiaozhenting at 20250911 start */
#include <linux/power/gxy_psy_sysfs.h>
/* Tab A11_BOS code for P250902-07125 by xiaozhenting at 20250911 end */

#define SCV89601P_PD_DRV_VER      "2.0"
...
int scv89601p_pd_get_alert_status_and_mask(struct tcpc_device *tcpc,
    uint32_t *alert, uint32_t *mask)
{
    ...
    *alert |= v2 << 16;
    ...
    scv89601p_pd_get_alert_mask(tcpc, mask);

    return 0;
}

static int scv89601p_pd_get_power_status(struct tcpc_device *tcpc)
{
    int ret;
    struct scv89601p_pd *sc = tcpc_get_dev_data(tcpc);
	...
	pr_info("vbus_present %d vbus_safe0v: %d",tcpc->vbus_present, tcpc->vbus_safe0v);
    return ret;
}
...
static struct tcpc_ops scv89601p_pd_tcpc_ops = {
    ...
    .get_alert_status_and_mask = scv89601p_pd_get_alert_status_and_mask,
...
{
    struct device_node __maybe_unused *np = dev->of_node;
...
static void scv89601p_pd_i2c_remove(struct i2c_client *client)
{
    struct scv89601p_pd *sc = i2c_get_clientdata(client);
 ...
 * 2.0
 * (1) Passed full PD certification in at least one project
 */