...
#include "a0702wxdepthov02f_mipi_raw.h"


#define PFX "a0702wxdepthov02f_mipi_raw_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = A0702WXDEPTHOV02F_SENSOR_ID,
	...
	.normal_video = {
		.pclk = 16500000,            //record different mode's pclk
		.linelength = 430,            //record different mode's linelength
		.framelength = 1598,            //record different mode's framelength
		.startx = 0,                    //record different mode's startx of grabwindow
		.starty = 0,                    //record different mode's starty of grabwindow
		.grabwindow_width = 1600,        //record different mode's width of grabwindow
		.grabwindow_height = 1200,        //record different mode's height of grabwindow
	...
	.hs_video = {
		.pclk = 16500000,            //record different mode's pclk
		.linelength = 430,            //record different mode's linelength
		.framelength = 1278,            //record different mode's framelength
		.startx = 0,                    //record different mode's startx of grabwindow
		.starty = 0,                    //record different mode's starty of grabwindow
		.grabwindow_width = 1600,        //record different mode's width of grabwindow
...
static void set_dummy(void)
{
	...
	write_cmos_sensor(0x14, (imgsensor.frame_length - imgsensor.vblank_convert) >> 8);
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if((gain >= 0x40) && (gain <= (15*0x40))) //base gain = 0x40
	{
		...
		if(iReg<=0x10)
		{
			...
			write_cmos_sensor(0xfe, 0x02);	//fresh
			pr_info("A0702WXDEPTHOV02F_MIPI_RAW_SetGain = 16");
		}
		else if(iReg>= 0xf8)//gpw
		{
			...
			write_cmos_sensor(0xfe, 0x02);	//fresh
			pr_info("A0702WXDEPTHOV02F_MIPI_RAW_SetGain = 160");
		}
		else
		{
			write_cmos_sensor(0xfd, 0x01);
			...
			write_cmos_sensor(0xfe, 0x02);	//fresh
			pr_info("A0702WXDEPTHOV02F_MIPI_RAW_SetGain = %d",iReg);
		}
	}
	else
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			pr_info("i2c write id(0) : 0x%x, sensor id: 0x%x,,sensor_id(0x%x)\n",imgsensor.i2c_write_id, *sensor_id,imgsensor_info.sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
			pr_info("i2c write id(1) : 0x%x, sensor id: 0x%x,,sensor_id(0x%x)\n",imgsensor.i2c_write_id, *sensor_id,imgsensor_info.sensor_id);
			return ERROR_NONE;
			}
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    imgsensor.vblank_convert = 1220; //for 1600*1200 30fps
	imgsensor.pclk = imgsensor_info.pre.pclk;
	imgsensor.line_length = imgsensor_info.pre.linelength;
	imgsensor.frame_length = imgsensor_info.pre.framelength;
	imgsensor.min_frame_length = imgsensor_info.pre.framelength;
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    imgsensor.vblank_convert = 1220; //for 1600*1200 24fps
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    imgsensor.vblank_convert = 920; //for 1600*900 30fps
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    imgsensor.vblank_convert = 920; //for 1600*900 24fps
...
UINT32 A0702WXDEPTHOV02F_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
