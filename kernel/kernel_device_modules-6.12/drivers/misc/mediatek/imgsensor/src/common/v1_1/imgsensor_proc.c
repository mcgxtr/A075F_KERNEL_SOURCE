...
/* Tab A9 code for AX6739W-29 by zhouying at 20240613 start */
    ...
    (struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    char buf[255] = {0};
    int rc = 0;
    pr_info("[cameradebug-sninfo] E");
    snprintf(buf, 255,
	"rear_camera:%s:%s\n",
	hq_cameraSNModuleInfo[0],hq_camerachipSN);

    rc = simple_read_from_buffer(page, size, ppos, buf, strlen(buf));
    pr_info("[cameradebug-sninfo] X");
...
/* Tab A9 code for AX6739W-29 by zhouying at 20240613 end */
