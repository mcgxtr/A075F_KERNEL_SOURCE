 ...
 * Filename:
 * ---------
 *	 a1103cxtbacksc820cs_mipi_raw.c
 *
 * Project:
 * --------
 *     ALPS
...
#include "a1103cxtbacksc820cs_mipi_raw.h"

#define PFX "a1103cxtbacksc820cs_mipi_raw"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = A1103CXTBACKSC820CS_SENSOR_ID,
...
static void set_max_framerate(UINT16 framerate, kal_bool min_framelength_en)
{
	...
	if (imgsensor.frame_length > imgsensor_info.max_frame_length){
		...
		imgsensor.line_length = imgsensor.pclk / framerate * 10 / imgsensor_info.max_frame_length;
		}
	imgsensor.dummy_line = imgsensor.frame_length - imgsensor.min_frame_length;
 ...
 *
 *************************************************************************/
static void set_short_shutter(kal_uint32 shutter)
{
...
static void set_long_exposure(kal_uint32 shutter)
{
	kal_uint16 longExpHts = 0;
	kal_int32 t_line = 13333; //13.333*1000
	kal_int32 longExpTime;
	kal_uint16 longExpHtsMin  = 0x6e0;
	kal_uint16 longExpHtsMax  = 0xfff0;

	longExpTime = (t_line*shutter)/1000;
	longExpHts = longExpTime * 264/65496; // 增加四舍五入

	if(longExpHts > longExpHtsMax)
		longExpHts = longExpHtsMax;
	else if(longExpHts < longExpHtsMin)
		longExpHts = longExpHtsMin;

	write_cmos_sensor(0x320e, 0x7f);
	write_cmos_sensor(0x320f, 0xf0);
	write_cmos_sensor(0x3e00, 0x0f);
	write_cmos_sensor(0x3e01, 0xfd);
	write_cmos_sensor(0x3e02, 0x80);

	write_cmos_sensor(0x320c, longExpHts >> 8);
	write_cmos_sensor(0x320d, longExpHts & 0xFF);

	LOG_INF("Exit! longExpHts = %d, longExpTime = %d\n", longExpHts, longExpTime);
}
static void set_shutter(kal_uint32 shutter)
{
	if(shutter >=imgsensor_info.max_frame_length - imgsensor_info.margin)
	{
		set_long_exposure(shutter);
		LOG_INF("Exit! set_long_exposure shutter = %d\n", shutter);
	}
	else
	{
		set_short_shutter(shutter);
		LOG_INF("Exit! set_short_shutter shutter = %d\n", shutter);
	}
}
...
UINT32 A1103CXTBACKSC820CS_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
