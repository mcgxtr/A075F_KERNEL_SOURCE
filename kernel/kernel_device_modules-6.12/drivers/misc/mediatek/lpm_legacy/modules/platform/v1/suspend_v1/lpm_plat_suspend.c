...
/*Tab A9 code for AX6739W-30 add by qiutong at 20250617 start*/
#include <linux/timer.h>
#include <linux/timex.h>
#include <linux/rtc.h>
/*Tab A9 code for AX6739W-30 add by qiutong at 20250617 end*/
...
#define SYS_WAKEUP_COUNT_MAX    4096
static int sys_lpm_power[SYS_WAKEUP_COUNT_MAX] = {0}; // 32KB
static long sys_lpm_current = 0;
static int sys_wakeup_count = 0;
static int sys_lpm_average_current = 0;
#define ABS_POWER(x)  \
        ((x < 0) ? -x : x)
...
{
    int i = 0;
    seq_printf(m, "[power]sys_wakeup_count=%d\n", sys_wakeup_count);
    for (i = 0; i < sys_wakeup_count && i < SYS_WAKEUP_COUNT_MAX; i++) {
        seq_printf(m, "[power]sys_lpm_power[%d]=%duA\n", i, sys_lpm_power[i]);
        if (sys_lpm_power[i] == 0) {
            break;
        }
    }
...
{
    return single_open(file, power_proc_show, inode->i_private);
}

static const struct proc_ops power_proc_fops = {
    .proc_open = power_proc_open,
    .proc_read = seq_read,
    .proc_lseek = seq_lseek,
    .proc_release = single_release,
};

static inline void power_get_lowpower_current(void);
...
static inline void power_get_lowpower_current(void)
{
    ...
    /*
    if (!power_proc_entry) {
        power_proc_entry = proc_create("lowpower", 0444, NULL, &power_proc_fops);
        if (!power_proc_entry) {
            pr_err("Failed to create /proc/lowpower\n");
        }
        pr_info("create /proc/lowpower\n");
    }
    */

    ret_power = power_get_sys_battery_current(&curr_now, &curr_avg, 0);
    pr_info("[power][%d]curr_now=%duA, %d\n", __LINE__, curr_now, ret_power);
    ret_power = power_get_sys_battery_current(&curr_now, &curr_avg, 1);
    pr_info("[power][%d]curr_avg=%duA, %d\n", __LINE__, curr_avg, ret_power);

    if ((curr_now > -1 || curr_avg > -1) && (curr_now != 0 || curr_avg != 0)) {
        pr_info("[power][%d]Battery Charging\n", __LINE__);
    } else {
        if (ABS_POWER(curr_now) < ABS_POWER(curr_avg)) {
            if (curr_now == 0) {
                if (sys_wakeup_count > (SYS_WAKEUP_COUNT_MAX - 1)) {
                    sys_lpm_power[SYS_WAKEUP_COUNT_MAX - 1] = ABS_POWER(curr_avg);
                } else {
                    sys_lpm_power[sys_wakeup_count] = ABS_POWER(curr_avg);
                }
                sys_lpm_current += ABS_POWER(curr_avg);
            } else {
                if (sys_wakeup_count > (SYS_WAKEUP_COUNT_MAX - 1)) {
                    sys_lpm_power[SYS_WAKEUP_COUNT_MAX - 1] = ABS_POWER(curr_now);
                } else {
                    sys_lpm_power[sys_wakeup_count] = ABS_POWER(curr_now);
                }
                sys_lpm_current += ABS_POWER(curr_now);
            }
        } else {
            if (curr_avg == 0) {
                if (sys_wakeup_count > (SYS_WAKEUP_COUNT_MAX - 1)) {
        ...
        for (i = 0; i <= sys_wakeup_count && i < SYS_WAKEUP_COUNT_MAX; i++) {
            pr_debug("[power][%d]sys_lpm_power[%d]=%duA\n", __LINE__, i, sys_lpm_power[i]);
            if (sys_lpm_power[i] == 0) {
                break;
            }
        }
        sys_wakeup_count++;
        sys_lpm_average_current = sys_lpm_current / sys_wakeup_count;
    }
    pr_info("[power][%d]lpm_avg_current=%duA, sys_lpm_current=%ld, resume_count=%d\n",
            __LINE__, sys_lpm_average_current, sys_lpm_current, sys_wakeup_count);
}
					...
					const struct lpm_issuer *issuer)
{
	...
	pr_info("PM:suspend resume UTC time :%d-%d-%d %d:%d:%d \n",
                tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour + 8, tm.tm_min, tm.tm_sec);
	/*Tab A9 code for AX6739W-30 add by qiutong at 20250617 end*/
        /* TabA11_BOS code for AX7800W-22 by gaochao at 20250820 start */
        power_get_lowpower_current();
        /* TabA11_BOS code for AX7800W-22 by gaochao at 20250820 end */
...
int __init lpm_model_suspend_init(void)
{
	...
	int i;
        /* HST11_BOS code for AX7800W-22 by gaochao at 20250820 start */
        struct proc_dir_entry *power_proc_entry = NULL;
        /* HST11_BOS code for AX7800W-22 by gaochao at 20250820 end */
