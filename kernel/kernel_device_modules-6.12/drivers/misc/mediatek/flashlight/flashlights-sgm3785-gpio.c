...
/* HS07 V code for HS07-19  by zhongbin at 2025/03/11 start */
...
/* HS07 V code for HS07-19  by zhongbin at 2025/03/11 end */
...
// hs07 code for HS07-19 by jianghongyan at 20250428 start
static const int sgm3785_current[LEVEL_MAX] = {
    34, 68, 103, 137, 171, 206, 240, 274, 309, 343, 377,
    412, 446, 481, 515, 549, 584, 618, 652, 687, 721, 755,
    790, 824, 858, 893, 927, 962, 996, 1030, 1065
};
// hs07 code for HS07-19 by jianghongyan at 20250428 end
...
static int sgm3785_set_torch_mode(int level)
{
    ...
    pr_info("sgm3785 torch_mode enter level=%d\n",level);
...
// hs07 code for HS07-19 by jianghongyan at 20250428 start
    case FLASH_IOC_GET_DUTY_NUMBER:
        ...
        fl_arg->arg = LEVEL_MAX;
        break;
    case FLASH_IOC_GET_DUTY_CURRENT:
        fl_arg->arg = sgm3785_current[fl_arg->arg];
    ...
    case FLASH_IOC_SET_ONOFF:              //115
        ...
        if (fl_arg->arg == 1) {
            switch(flashlight_mode) {
                case MODE_FLASH: {
                    /* HS07 V code for HS07-1960  by zhongbin at 2025/05/22 start */
                    /* max timeout value 670*/
                    if (sgm3785_gpio_timeout_ms == 0) {
                        sgm3785_gpio_timeout_ms = 500;
                    }
                    /* HS07 V code for HS07-1960  by zhongbin at 2025/05/22 end */
                    s = sgm3785_gpio_timeout_ms / 1000;
                    ns = sgm3785_gpio_timeout_ms % 1000 * 1000000;
                    ktime = ktime_set(s, ns);
                    hrtimer_start(&sgm3785_gpio_timer, ktime,
                            HRTIMER_MODE_REL);
                    /* HS07 V code for HS07-1960  by zhongbin at 2025/05/26 start */
                    if(current_level <= 6 ){
                        pr_info("flashmode run preflash_mode timeout_ms = %d, current_level = %d\n",sgm3785_gpio_timeout_ms,current_level);
                        sgm3785_set_preflash_mode(current_level);
                    }else{
                        pr_info("flashmode run flash_mode timeout_ms = %d, current_level = %d\n",sgm3785_gpio_timeout_ms,current_level);
                        sgm3785_set_flash_mode(current_level);
                    }
                    /* HS07 V code for HS07-1960  by zhongbin at 2025/05/26 end */
                }
                    break;
                case MODE_PREFLASH:
                    sgm3785_set_preflash_mode(current_level);
...
static void sgm3785_gpio_remove(struct platform_device *pdev)
{
    struct sgm3785_gpio_platform_data *pdata = dev_get_platdata(&pdev->dev);
    int i;
...
int flashlight_sgm3785_gpio_init(void) {
    int ret;
...
void flashlight_sgm3785_gpio_exit(void) {
    pr_debug("Exit start.\n");

#ifndef CONFIG_OF
    platform_device_unregister(&sgm3785_gpio_platform_device);
#endif
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Focus.HQ <zhongbin9@huaqin.com>");
