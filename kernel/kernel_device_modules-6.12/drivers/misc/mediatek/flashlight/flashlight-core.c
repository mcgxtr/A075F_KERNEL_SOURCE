...
#include "flashlights-sgm3785-gpio.h"
...
static void hs07_create_torch_node(void);
#endif
...
/* HS07 V code for HS07-19  by zhongbin at 2025/02/28 start */
#if defined(CONFIG_CUSTOM_PROJECT_HS07)
/******************************************************************************
 * File operations
 *****************************************************************************/
static long k_hs07_flashlight_ioctl(
		struct file *file, unsigned int cmd, unsigned long arg)
{
	struct flashlight_user_arg fl_arg;
	struct flashlight_dev_arg fl_dev_arg;
	...
	int type, ct, part;
	int ret = 0;
    pr_info("k_hs07_flashlight_ioctl start\n");
	memset(&fl_arg, 0, sizeof(struct flashlight_user_arg));
	if (NULL == memcpy(&fl_arg, (struct flashlight_user_arg*)arg, sizeof(struct flashlight_user_arg))) {
		pr_info("Failed copy arguments from flashlight node\n");
		return -EFAULT;
	}
	...
	mutex_unlock(&fl_mutex);
	if (!fdev) {
		pr_info_ratelimited("Find no flashlight device 2\n");
		return -EINVAL;
	}
	...
	switch (cmd) {
		case FLASH_IOC_SET_DUTY:
			pr_info("[flashlight i]FLASH_IOC_SET_DUTY(%d,%d,%d): %d\n",
					type, ct, part, fl_arg.arg);
			mutex_lock(&fl_mutex);
			ret = fl_set_level(fdev, fl_arg.arg);
			mutex_unlock(&fl_mutex);
		...
		case FLASH_IOC_SET_ONOFF:
			pr_info("[flashlight i]FLASH_IOC_SET_ONOFF(%d,%d,%d): %d\n",
					type, ct, part, fl_arg.arg);
			mutex_lock(&fl_mutex);
			ret = fl_enable(fdev, fl_arg.arg);
			mutex_unlock(&fl_mutex);
		...
		default:
			pr_info("[flashlight i]Failed with no flashlight ops %d\n", cmd);
			return -ENOTTY;
			break;
	}
...
#endif
/* HS07 V code for HS07-19  by zhongbin at 2025/02/28 end */
...
/* HS07 V code for HS07-19  by zhongbin at 2025/02/28 start */
#if defined(CONFIG_CUSTOM_PROJECT_HS07)
static int hs07_torch_level=0;

static ssize_t hs07_torch_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
    return scnprintf(buf, PAGE_SIZE, "%d\n", hs07_torch_level);
}

static ssize_t hs07_torch_store(struct device *dev,struct device_attribute *attr,
                    const char *buf, size_t len)
{
    int set_torch_level;
	struct flashlight_user_arg fl_arg;
	int ret;
    if (kstrtoint(buf, 0, &hs07_torch_level))
    {
        pr_err("[flashlight E] flashlight add the torch stength error");
        return -EINVAL;
    }
	fl_arg.ct_id = 1;
	fl_arg.type_id = 1;

    pr_err("[flashlight E] flashlight set torch level is %d", hs07_torch_level);
	switch(hs07_torch_level)
    {
        case 1001:
            set_torch_level = 1;
            break;
	...
	ret = k_hs07_flashlight_ioctl(NULL, FLASH_IOC_SET_DUTY, (unsigned long)&fl_arg);

	fl_arg.arg = 1;
	ret = k_hs07_flashlight_ioctl(NULL, FLASH_IOC_SET_ONOFF, (unsigned long)&fl_arg);

	pr_err("[flashlight E] flashlight set torch level %d ret = %d", set_torch_level, ret);

	return len;
}

struct class *hs07_torch_class;
struct device *hs07_torch_dev1;

static DEVICE_ATTR(rear_flash, 0644, hs07_torch_show, hs07_torch_store);
static DEVICE_ATTR(rear_torch_flash, 0644, hs07_torch_show, hs07_torch_store);

static void hs07_create_torch_node(void)
{
    hs07_torch_class = class_create("camera");
    hs07_torch_dev1 = device_create(hs07_torch_class, NULL, 0, NULL, "flash");

    if (device_create_file(hs07_torch_dev1, &dev_attr_rear_flash) < 0 || device_create_file(hs07_torch_dev1, &dev_attr_rear_torch_flash) < 0)
    {
        device_destroy(hs07_torch_class, 0);
        class_destroy(hs07_torch_class);
    }
}
#endif
/* HS07 V code for HS07-19  by zhongbin at 2025/02/28 end */
...
/* HS07 V code for HS07-19  by zhongbin at 2025/02/28 start */
#if defined(CONFIG_CUSTOM_PROJECT_HS07)
    hs07_create_torch_node();
    pr_err("[flashlight E] hs07 creat torch node OK!");
#endif
/* HS07 V code for HS07-19  by zhongbin at 2025/02/28 end */
	pr_debug("Probe done\n");
...
static int __init flashlight_init(void)
{
    ...
    ret = flashlight_sgm3785_gpio_init(); 
    if (ret < 0) {
        pr_err("Failed to initialize sgm3785 - gpio\n");
        return ret;
    }
...
#ifndef CONFIG_OF
    platform_device_unregister(&flashlight_platform_device);
#endif
    platform_driver_unregister(&flashlight_platform_driver);
    flashlight_sgm3785_gpio_exit(); 
