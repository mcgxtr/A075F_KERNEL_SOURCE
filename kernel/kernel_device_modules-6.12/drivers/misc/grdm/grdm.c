...
/* Tab A9 code for SR-AX6739A-01-393 by gaochao at 20230613 end */
        ...
        pr_err("[%s]l=%d: buf is %s, len is %zu, grdm_is_open is %d\n",
						...
						size_t count, loff_t *ppos)
{
	...
	if (grdm_data.grdm_is_open == GDRM_OPEN) {
		if ((copy_to_user(buf, "1", 1)) != 0) {
			pr_err("Failed to copy '1' to user\n");
		}
	} else {
		if ((copy_to_user(buf, "0", 1)) != 0) {
			pr_err("Failed to copy '1' to user\n");
		}
	}
	return count;
...
static int grdm_probe(struct platform_device *pdev)
{
    ...
    grdm_node_class = class_create("grdm_node_class");
    if (IS_ERR(grdm_node_class)) {
        pr_err("fail to create grdm class,ret is %ld\n",PTR_ERR(grdm_node_class));
    ...
    if (IS_ERR(grdm_node_device)) {
        pr_err("fail to create grdm device,ret is %ld\n",PTR_ERR(grdm_node_device));
...
static void grdm_remove(struct platform_device *pdev)
{
#if defined(SECURITY_IC_POWER_SUPPLY_VIA_LDO_CTL) // ldo control, otherwise always on
    int ldo_en_stat = 0;
#endif
