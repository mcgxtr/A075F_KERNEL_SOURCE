...
int hbm_map_level(int level, int normal_max_bl, int hbm_max_bl) {
    ...
    if (level <= 255) {
        return (normal_max_bl * level) / 255;
    } else {
        return normal_max_bl + ((hbm_max_bl - normal_max_bl) * (level - 255)) / 255;
    }
}
EXPORT_SYMBOL(hbm_map_level);
...
EXPORT_SYMBOL_GPL(panel_bias_i2c_write_bytes);
...
int panel_bias_i2c_probe(struct i2c_client *client)
{
...
static void panel_bias_i2c_remove(struct i2c_client *client)
{
    pr_info("[LCM][I2C] bias %s\n", __func__);
