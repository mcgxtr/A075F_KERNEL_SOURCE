...
extern int mtkfb_set_esd_backlight_level(void);
bool esd_setbacklight_flag = false;
#endif
...
static int mtk_drm_esd_check_worker_kthread(void *data)
{
	...
	while (1) {
		...
		do {
			...
			/*HS07 code for AL7761C-1 by jiangyue at 20250728 start*/
			...
			/*HS07 code for AL7761C-1 by jiangyue at 20250728 end*/
			if (ret < 0) {
		...
		#if defined(CONFIG_CUSTOM_PROJECT_HS07) || defined(CONFIG_CUSTOM_PROJECT_HST11)
		if (esd_setbacklight_flag) {
			DDPMSG("%s: esd need_setbacklight\n", __func__);
			mtkfb_set_esd_backlight_level();
			esd_setbacklight_flag = false;
		}
		#endif//CONFIG_CUSTOM_PROJECT_HS07
