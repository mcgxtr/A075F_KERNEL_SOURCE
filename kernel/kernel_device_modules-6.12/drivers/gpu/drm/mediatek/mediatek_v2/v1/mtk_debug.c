...
/*HS07 code for AL7761C-1 by jiangyue at 20250721 start*/
#if defined(CONFIG_CUSTOM_PROJECT_HS07)
#include <video/mipi_display.h>
#endif//CONFIG_CUSTOM_PROJECT_HS07
/*HS07 code for AL7761C-1 by jiangyue at 20250721 end*/
...
#define DSI_PANEL_ESD_DEFAULT_BL_LEVEL  1638;//4096 * 40% = 1638
static unsigned int gs_esd_recovery_level = DSI_PANEL_ESD_DEFAULT_BL_LEVEL;
#endif
/*HS07 code for AL7761C-1 by jiangyue at 20250721 end*/
...
#if defined(CONFIG_CUSTOM_PROJECT_HS07) || defined(CONFIG_CUSTOM_PROJECT_HST11)
int mtkfb_set_esd_backlight_level(void)
{
	DDPMSG("%s: gs_esd_recovery_level %d\n", __func__, gs_esd_recovery_level);
	return mtkfb_set_backlight_level(gs_esd_recovery_level, 0, 0x1<<SET_BACKLIGHT_LEVEL);
}
EXPORT_SYMBOL(mtkfb_set_esd_backlight_level);
#endif
/*HS07 code for AL7761C-1 by jiangyue at 20250721 end*/
...
/*HS07 code for AL7761C-1 by jiangyue at 20250721 start*/
#if defined(CONFIG_CUSTOM_PROJECT_HS07)
void ddic_dsi_write_reg_cmd(u8 addr, u8 tx_len,u8 *tx_buf)
{
	unsigned int i = 0;
	int ret = 0;
	struct mtk_ddic_dsi_msg *cmd_msg = vmalloc(sizeof(struct mtk_ddic_dsi_msg));
	u8 tx[DDIC_READ_WRITE_BUF_LENGTH] = {0};
	DDPMSG("%s start \n", __func__);
	tx_len = tx_len + 1;////add addr
	memcpy(tx, tx_buf, tx_len * sizeof(u8));
	if (!cmd_msg) {
		DDPPR_ERR("cmd msg is NULL\n");
		return;
	}
	...
	switch (tx_len) {
		case 0x01:
			cmd_msg->type[0] = MIPI_DSI_DCS_SHORT_WRITE;
			break;
		case 0x02:
			cmd_msg->type[0] = MIPI_DSI_DCS_SHORT_WRITE_PARAM;
			break;
		default:
			cmd_msg->type[0] = MIPI_DSI_DCS_LONG_WRITE;
			break;
	}
	cmd_msg->channel = 0;
	cmd_msg->flags |= MIPI_DSI_MSG_USE_LPM;
	...
	tx[0] = addr;
	cmd_msg->tx_buf[0] = tx;
	cmd_msg->tx_len[0] = tx_len;
	for (i = 0; i < (int)cmd_msg->tx_len[0]; i++) {
		DDPMSG("write_ddic lcm type[0]=0x%02x, tx_buf[0]--byte:%d,val:0x%02x\n",
			cmd_msg->type[0], i, *(char *)(cmd_msg->tx_buf[0] + i));
	}
	ret = mtk_ddic_dsi_send_cmd(cmd_msg, true, true);
	if (ret != 0) {
		DDPPR_ERR("mtk_ddic_dsi_send_cmd error\n");
...
void ddic_dsi_read_reg_cmd(u8 addr, u8 rx_len)
{
	unsigned int i = 0;
	int ret = 0;
	struct mtk_ddic_dsi_msg *cmd_msg = vmalloc(sizeof(struct mtk_ddic_dsi_msg));
	...
	if (!cmd_msg)
		return;
	if (rx_len > DDIC_READ_WRITE_BUF_LENGTH) {
		DDPPR_ERR("%s the length read is too long\n", __func__);
		return;
	}
	DDPMSG("%s read ddic add:0x%02x, length=0x%02x\n", __func__, addr,rx_len);
	memset(cmd_msg, 0, sizeof(struct mtk_ddic_dsi_msg));
	/* Read 0x0A = 0x1C */
	cmd_msg->channel = 0;
	cmd_msg->tx_cmd_num = 1;
	cmd_msg->type[0] = MIPI_DSI_DCS_READ;
	tx[0] = addr;
	cmd_msg->tx_buf[0] = tx;
	cmd_msg->tx_len[0] = 1;
	cmd_msg->rx_cmd_num = 1;
	cmd_msg->rx_buf[0] = kmalloc(rx_len * sizeof(unsigned char),
			GFP_ATOMIC);
	memset(cmd_msg->rx_buf[0], 0, rx_len);
	cmd_msg->rx_len[0] = rx_len;
	ret = mtk_ddic_dsi_read_cmd(cmd_msg, true);
	if (ret != 0) {
		DDPPR_ERR("%s error\n", __func__);
		goto  done;
	}
	for (i = 0; i < (int)cmd_msg->rx_len[0]; i++) {
		DDPMSG("read_ddic lcm addr:0x%02x--byte:%d,val:0x%02x\n",
			*(char *)(cmd_msg->tx_buf[0]), i, *(char *)(cmd_msg->rx_buf[0] + i));
	}
done:
	for (i = 0; i < cmd_msg->rx_cmd_num; i++)
		kfree(cmd_msg->rx_buf[i]);
...
/*HS07 code for AL7761C-1 by jiangyue at 20250721 start*/
#if defined(CONFIG_CUSTOM_PROJECT_HS07)
	} else if (strncmp(opt, "write_ddic:", 11) == 0) {
		unsigned int ret, parse_data = 0;
		u8 addr, tx_len = 0;
		u8 tx_buf[DDIC_READ_WRITE_BUF_LENGTH] = {0};
		int i = 0;
		ret = sscanf(opt, "write_ddic:%x,%x",&addr,&tx_len);
		if (ret != 2) {
			DDPPR_ERR("%d error to parse cmd %s\n",
				__LINE__, opt);
			return;
		}
		if (tx_len > (sizeof(tx_buf)-1)) {
			DDPPR_ERR("%d tx_len too long,can not send\n",
				__LINE__);
			return;
		}
		tx_buf[0] = addr;
		...
		while (parse_data < tx_len && sscanf(opt, "%x",&tx_buf[parse_data + 1]) == 1) {
			parse_data++;
			opt = strchr(opt,',');
			if (!opt) {
				break;
		...
		if (parse_data != tx_len) {
			DDPPR_ERR("%d parse_data(%d) no match tx_len(%d)\n",__LINE__,parse_data,tx_len);
			return;
		}
		DDPMSG("write_ddic:0x%02x,0x%02x",addr,tx_len);
		for(i = 1; i <= tx_len; i++) {
			DDPMSG("byte[%d]=0x%02x", i-1, tx_buf[i]);
		}
		ddic_dsi_write_reg_cmd(addr,tx_len,tx_buf);
	} else if (strncmp(opt, "read_ddic:", 10) == 0) {
		u8 addr,rx_len = 0;
		unsigned int ret = 0;
		ret = sscanf(opt, "read_ddic:%x,%x\n", &addr,&rx_len);
		if (ret != 2) {
			DDPPR_ERR("%d error to parse cmd %s\n",
				__LINE__, opt);
			return;
		}
		DDPMSG("read_ddic:0x%02x,0x%02x\n", addr,rx_len);
		ddic_dsi_read_reg_cmd(addr,rx_len);
#endif//CONFIG_CUSTOM_PROJECT_HS07
/*HS07 code for AL7761C-1 by jiangyue at 20250721 end*/
