...
END:
    ...
    if (parse_succ == -1) {
        ...
        strncpy(g_str_crtra_ver, "No Version", 12);
    } else {
...
static int himax_self_test_data_init(void)
{
    ...
    char *file_name = ts->himax_csv_name;
    int setting_sz = -1;
    int ret = HX_INSP_OK;
    int err = 0;
    int i = 0;
...
/*HS07 code for SR-AL7761A-01-538 by huangyin at 20250318 start*/
    ...
    memset(g_file_path, 0, 256);
    snprintf(g_file_path, (int)(strlen(HX_RSLT_OUT_PATH)
            + strlen(get_date_time_str()) + strlen(rslt ? "FAIL":"PASS") + strlen(hx_s_ts->module_name) + LENGTH_OF_DATA_NAME),
            "%s%s_%s_%s.txt", HX_RSLT_OUT_PATH, rslt ? "FAIL":"PASS", get_date_time_str(), hx_s_ts->module_name);
    g_file_w_flag = true;
    if (g_file_w_flag) {
        I("Start to write fs!\n");
        if (hx_open_file(g_file_path) != NO_ERR) {
...
void himax_inspection_init(void)
{
    ...
    hx_s_core_fp._chip_self_test_for_tp = himax_chip_self_test_for_tp;
}
