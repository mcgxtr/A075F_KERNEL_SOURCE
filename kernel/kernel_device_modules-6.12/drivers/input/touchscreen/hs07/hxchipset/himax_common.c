...
/*HS07 code for SR-AL7761A-01-538 by huangyin at 20250318 start*/
static int himax_self_test_for_tp(void)
{
    int result = 0;
    I("%s: enter, %d\n", __func__, __LINE__);
    ...
    himax_int_enable(0);/* disable irq */
    if (hx_s_core_fp._chip_self_test != NULL) {
        result = hx_s_core_fp._chip_self_test_for_tp();
    } else {
        E("%s: Doesn't turn on inspection function!\n", __func__);
    }
...
/*HS07 code for SR-AL7761A-01-538 by huangyin at 20250318 end*/
        ...
        size_t len, loff_t *pos)
{
    ...
    if (buf[0] == '0') {
        ...
        ts->SMWP_enable = 1;
        /*HS07 code for HS07-2937 by huangyin at 20250513 start*/
        if (hx_s_ts->IRQ_WAKEUP_STA == 0) {
            irq_set_irq_wake(hx_s_ts->hx_irq, 1);
            hx_s_ts->IRQ_WAKEUP_STA = 1;
        }
        /*HS07 code for HS07-2937 by huangyin at 20250513 end*/
    } else
        return -EINVAL;
...
skip_stylus_operation:
    ...
    for (i = 0; i < ts->nFinger_support; i++) {
        ...
        && y <= ts->pdata->abs_y_max) {
            ...
            /*HS07 code for SR-AL7761A-01-534 by huangyin at 20250317 start*/
            ...
            /*HS07 code for SR-AL7761A-01-534 by huangyin at 20250317 end*/
        }
    }
...
void himax_ts_work(struct himax_ts_data *ts)
{
    ...
    int ts_path = 0;
/*HS07 code for HS07-652 by huangyin at 20250415 start*/
#if defined(CONFIG_PM) && defined(HX_PATCH_COMERR_PM)
    int ret = 0;
#endif
/*HS07 code for HS07-652 by huangyin at 20250415 end*/
    if (!ts->initialized) {
...
static void himax_boot_upgrade(struct work_struct *work)
{
    ...
    uint8_t write_data[DATA_LEN_4] = {0};
    ...
    if (g_boot_upgrade_flag == true) {
        if (i_update_FW() <= 0) {
            ...
            /*HS07 code for HS07-3466 by huangyin at 20250526 start*/
            I("%s: Update FW success, write boot flag start.\n", __func__);
            hx_parse_assign_cmd(hx_s_ic_setup._data_boot_noise_flag,
            write_data, DATA_LEN_4);
            hx_s_core_fp._register_write(hx_s_ic_setup._addr_boot_noise_flag,
            write_data,DATA_LEN_4);
            I("%s: write boot flag end.\n", __func__);
            /*HS07 code for HS07-3466 by huangyin at 20250526 end*/
...
static int himax_earphone_notifier_callback(struct notifier_block *nb, unsigned long event, void *ptr)
{
    ...
    I("Current state: typec_plugin=%d, round_plugin=%d, hp_en=%d\n",
        g_tp_typec_earphone_in, g_tp_round_earphone_in, ts->hp_en);
...
static int himax_get_tp_module(void)
{
    ...
    if (NULL != strstr(panel_name, "hx83108d_xx_hkc")) {
        fw_num = MODEL_XX_HKC;
    }else
...
static void himax_update_module_info(void)
{
    ...
    {
        case MODEL_XX_HKC:
                strcpy(ts->module_name,"hx83108d_xx_hkc");
                strcpy(ts->himax_nomalfw_rq_name, "hx83108d_xx_hkc_firmware.bin");
                strcpy(ts->himax_mpfw_rq_name, "hx83108d_xx_hkc_mpfw.bin");
                strcpy(ts->himax_csv_name, "hx83108d_xx_hkc_criteria.csv");
                break;
        default:
                strcpy(ts->module_name,"unknown");
                strcpy(ts->himax_nomalfw_rq_name, "unknown.bin");
                strcpy(ts->himax_mpfw_rq_name, "unknown.bin");
                strcpy(ts->himax_csv_name, "unknown.csv");
                break;
    }
    hx_s_ts->boot_fw_name = ts->himax_nomalfw_rq_name;
}
/*HS07 code for SR-AL7761A-01-538 by huangyin at 20250318 end*/
static void tp_feature_interface(void)
{
    ...
    /*HS07 code for SR-AL7761A-01-538 by huangyin at 20250318 start*/
    gs_hx_lcm_info->ito_test = himax_self_test_for_tp;
    /*HS07 code for SR-AL7761A-01-538 by huangyin at 20250318 end*/
...
/*HS07 code for HS07-652 by huangyin at 20250415 start*/
#if defined(CONFIG_PM) && defined(HX_PATCH_COMERR_PM)
    init_completion(&hx_s_ts->pm_completion);
    hx_s_ts->pm_suspend = false;
#endif
/*HS07 code for HS07-652 by huangyin at 20250415 end*/
#if defined(HX_ZERO_FLASH)
 ...
 */
    ...
    /*HS07 code for AL7761C-1 by xiongwei at 20250728 start*/
    INIT_WORK(&ts->headset_work_queue, himax_earphone_workqueue);
    ts->earphone_notif.notifier_call = himax_earphone_notifier_callback;
    ret = earphone_notifier_register(&ts->earphone_notif);
    /*HS07 code for HS07-3467 by huangyin at 20250526 start*/
    if (ret) {
        ...
        I("Ready to register register earphone notifier: %d\n", ret);
    }
    /*HS07 code for HS07-3467 by huangyin at 20250526 end*/
...
err_headset_notify_failed:
    ...
    earphone_notifier_unregister(&ts->earphone_notif);
