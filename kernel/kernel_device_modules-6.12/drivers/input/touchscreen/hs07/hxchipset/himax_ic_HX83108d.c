...
#include "himax_ic_HX83108d.h"
#include "himax_modular.h"

static void hx83108d_chip_init(void)
{
    ...
    (hx_s_fwbin->addr_fw_ver_maj) = 49157;  /*0x00C005*/
    (hx_s_fwbin->addr_fw_ver_min) = 49158;  /*0x00C006*/
    (hx_s_fwbin->addr_cfg_ver_maj) = 49408;  /*0x00C100*/
    (hx_s_fwbin->addr_cfg_ver_min) = 49409;  /*0x00C101*/
    (hx_s_fwbin->addr_cid_ver_maj) = 49154;  /*0x00C002*/
    (hx_s_fwbin->addr_cid_ver_min) = 49155;  /*0x00C003*/
    (hx_s_fwbin->addr_cfg_table) = 0x10000;
}

struct ic_setup_collect g_hx83108d_setup = {
    ...
    ._addr_rawout_sel = HX83108D_ADDR_RAWOUT_SEL,
    ...
    /*HS07 code for HS07-3466 by huangyin at 20250526 start*/
    ._addr_boot_noise_flag    = ADDR_BOOT_NOISE_FLAG,
    ._data_boot_noise_flag    = DATA_BOOT_NOISE_FLAG,
    /*HS07 code for HS07-3466 by huangyin at 20250526 end*/
};


void hx83108d_burst_enable(uint8_t auto_add_4_byte)
{
...
int hx83108d_register_write(uint32_t addr, uint8_t *val, uint32_t len)
{
    // uint8_t tmp_addr[DATA_LEN_4];
    ...
    if (addr == ADDR_FLASH_SPI200_DATA)
        hx83108d_burst_enable(0);
    else if (len > DATA_LEN_4)
        hx83108d_burst_enable(1);
    else
        hx83108d_burst_enable(0);
...
static int hx83108d_register_read(uint32_t addr, uint8_t *buf, uint32_t len)
{
    uint8_t tmp_addr[DATA_LEN_4];
    uint8_t tmp_data[DATA_LEN_4];
...
void hx83108d_system_reset(void)
{
    ...
    do {
        ...
        hx83108d_register_read(ADDR_FLAG_RESET_EVENT,
            tmp_data, DATA_LEN_4);
...
static bool hx83108d_sense_off(bool check_en)
{
    uint8_t cnt = 0;
    uint8_t tmp_data[DATA_LEN_4];
    uint8_t tmp_addr[DATA_LEN_4];
    ...
    do {
        ...
        && tmp_data[0] != 0x87)) {
            hx_parse_assign_cmd(DATA_FW_STOP,
                tmp_data,
                DATA_LEN_4);
            hx83108d_register_write(ADDR_CTRL_FW,
                tmp_data,
                DATA_LEN_4);
        }

        /* check fw status */
        hx83108d_register_read(ADDR_CS_CENTRAL_STATE,
            tmp_data, DATA_LEN_4);
        ...
        hx83108d_register_read(ADDR_CTRL_FW, tmp_data, 4);
    ...
    do {
        ...
        if (tmp_data[0] == 0x0C) {
            ...
            hx83108d_register_write(ADDR_TCON_ON_RST,
                tmp_data, DATA_LEN_4);
             ...
             */
            hx_parse_assign_cmd(DATA_CLEAR, tmp_data, DATA_LEN_4);
            hx83108d_register_write(ADDR_ADC_ON_RST,
                tmp_data, DATA_LEN_4);
            usleep_range(1000, 1001);
...
#if defined(HX_ZERO_FLASH)
static void hx83108d_opt_hw_crc(void)
{
...
static void hx83108d_setup_opt_hw_crc(const struct firmware *fw)
{
    uint32_t tmp_p_addr;
...
static void hx83108d_opt_crc_clear(void)
{
    uint8_t data[DATA_LEN_4] = {0};
    ...
    hx_parse_assign_cmd(hx83108d_data_crc_s1, data, DATA_LEN_4);
    hx_s_core_fp._register_write(hx83108d_addr_crc_s1, data, DATA_LEN_4);

    hx_parse_assign_cmd(hx83108d_data_crc_s2, data, DATA_LEN_4);
    hx_s_core_fp._register_write(hx83108d_addr_crc_s2, data, DATA_LEN_4);

    hx_parse_assign_cmd(hx83108d_data_crc_s3, data, DATA_LEN_4);
    hx_s_core_fp._register_write(hx83108d_addr_crc_s3, data, DATA_LEN_4);

    do {
        hx_parse_assign_cmd(hx83108d_data_crc_s4,
            wrt_data, DATA_LEN_4);
        hx_s_core_fp._register_write(hx83108d_addr_crc_s4,
            wrt_data, DATA_LEN_4);

        usleep_range(1000, 1100);
        hx_s_core_fp._register_read(hx83108d_addr_crc_s4,
            data, DATA_LEN_4);
...
static void hx83108d_reload_to_active(void)
{
    uint8_t data[DATA_LEN_4] = {0};
...
static void hx83108d_resume_ic_action(void)
{
#if !defined(HX_RESUME_HW_RESET)
    hx83108d_reload_to_active();
#endif
}

#endif

static void hx83108d_sense_on(uint8_t FlashMode)
{
    uint8_t tmp_data[DATA_LEN_4];
    uint8_t tmp_addr[DATA_LEN_4];
    int retry = 0;
...
static void hx83108d_en_flash_access(uint8_t en)
{
    ...
    do {
        ...
        hx_s_core_fp._register_write(hx83108d_addr_en_flash_acc,
                tmp_data, DATA_LEN_4);
        usleep_range(10000, 10001);
        hx_s_core_fp._register_read(hx83108d_addr_en_flash_acc,
            back_data,
            DATA_LEN_4);
        I("%s: tmp_data[0]=%d, enable=%d, retry_cnt=%d\n",
...
int hx83108d_fts_ctpm_fw_upgrade_with_sys_fs(unsigned char *fw,
...
void hx83108d_flash_dump_func(uint8_t local_flash_command,
...
bool hx83108d_flash_lastdata_check(uint32_t size)
{
...
void hx83108d_init(void)
{
    ...
    hx_s_core_fp._fts_ctpm_fw_upgrade_with_sys_fs =
            hx83108d_fts_ctpm_fw_upgrade_with_sys_fs;
    hx_s_core_fp._flash_dump_func = hx83108d_flash_dump_func;
    hx_s_core_fp._flash_lastdata_check = hx83108d_flash_lastdata_check;
...
#endif
    hx_s_core_fp._system_reset = hx83108d_system_reset;
    ...
    hx_s_core_fp._burst_enable = hx83108d_burst_enable;
    hx_s_core_fp._register_read = hx83108d_register_read;
    hx_s_core_fp._register_write = hx83108d_register_write;

    hx_s_core_fp._sense_off = hx83108d_sense_off;
    hx_s_core_fp._sense_on = hx83108d_sense_on;
    hx_s_core_fp._chip_init = hx83108d_chip_init;

#if defined(HX_ZERO_FLASH)
    hx_s_core_fp._resume_ic_action = hx83108d_resume_ic_action;
    hx_s_core_fp._0f_reload_to_active = hx83108d_reload_to_active;
    hx_s_core_fp._en_hw_crc = NULL;
    hx_s_core_fp._setup_opt_hw_crc = hx83108d_setup_opt_hw_crc;
    hx_s_core_fp._opt_crc_clear = hx83108d_opt_crc_clear;
...
static bool hx83108d_chip_detect(void)
{
    uint8_t tmp_data[DATA_LEN_4];
    bool ret = false;
    int i = 0;
    ...
    if (hx83108d_sense_off(false) == false) {
        ret = false;
        E("%s:_sense_off Fail:\n", __func__);
        return ret;
    }
    for (i = 0; i < 5; i++) {
        ret = hx83108d_register_read(HX83108D_REG_ICID,
            tmp_data, DATA_LEN_4);
        ...
        && (tmp_data[2] == 0x10)
        && (tmp_data[1] == 0x8d)) {
            strncpy(hx_s_ts->chip_name,
                HX_83108D_SERIES_PWON, 30);
            hx_s_ic_data->ic_adc_num =
                HX83108D_DATA_ADC_NUM;
...
#endif
            hx83108d_init();
            hx_s_ic_setup = g_hx83108d_setup;
...
bool _hx83108d_init(void)
{
    bool ret = false;

    I("%s\n", __func__);
    ret = hx83108d_chip_detect();
    return ret;
}
