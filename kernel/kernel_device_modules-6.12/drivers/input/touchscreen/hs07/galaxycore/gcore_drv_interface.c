...
u32 flash_buf[2] = {0x10000,0x10000};
#endif
...
#ifdef CONFIG_GET_DATA_FROM_FLASH
    case IOC_FLASHDOWNLOAD_BIN:
        GTP_DEBUG("Start flashdownload GC7xxx_Flash_Op.bin");
        break;
    case IOC_START_READ_FLASH_DATA:
        GTP_DEBUG("Start read Flash data");
        ret = gcore_read_flash_data_proc();
        if(!ret)
            return 1;
        else
            return 2;

     case IOC_SET_FLASHREAD_PARAMETERS:
        GTP_DEBUG("Start Set flash read parameters");
        ret = copy_from_user(flash_buf, (void __user *)arg, sizeof(flash_buf));
        if (ret) {
            GTP_ERROR("Get parameters err:copy from user fail.");
            return -EPERM;
        }
        GTP_DEBUG("Flash read: lenth = 0x%x , addr = 0x%x",flash_buf[0],flash_buf[1]);
        gcore_set_flashread_parameters(flash_buf[0],flash_buf[1]);
        return ret;
#endif
...
/*HS07 code for HS-35 by huangyin at 20250226 start*/
...
/*HS07 code for HS-35 by huangyin at 20250226 end*/
...
#ifdef CONFIG_GET_DATA_FROM_FLASH
static ssize_t gtp_sysfs_flashdata_write(struct file *file,
...
static ssize_t gtp_sysfs_flashdata_read(struct file *file,
        struct kobject *kobj, struct bin_attribute *attr,
        char *buf, loff_t pos, size_t count)
{
    if(gdev_intf->read_addr_pos >= gdev_intf->flash_data_size){
        GTP_ERROR("No data to read :%d",gdev_fwu->read_addr_pos);
        if(gdev_intf->flash_data_buff){
            kfree(gdev_intf->flash_data_buff);
            GTP_ERROR("free flash_data_buff");
            gdev_intf->flash_data_buff = NULL;
        }
        return 0;
    }
    if(gdev_intf->read_flash_buff){
        memcpy(buf,gdev_intf->flash_data_buff+gdev_intf->read_addr_pos,GCORE_LENFTH_OF_FLASH_READ_ONCE);
        count = GCORE_LENFTH_OF_FLASH_READ_ONCE;
        //GTP_INFO("buf=0x%x 0x%x 0x%x 0x%x",buf[0],buf[1],buf[2],buf[3]);
        gdev_intf->read_addr_pos+=GCORE_LENFTH_OF_FLASH_READ_ONCE;
    }else{
        GTP_ERROR("read_flash_buff free!");
        count = 0;
    }
    //GTP_INFO("Read addr:%d",gdev_intf->read_addr_pos);
    return count;
}

static const struct bin_attribute flash_attr = {
    .attr = {
        .name = "flashdata",
        .mode = 0666,
    },
    .size = 0,
    .write = gtp_sysfs_flashdata_write,
    .read = gtp_sysfs_flashdata_read,
};
struct kobject *flash_kobj = NULL;
int gcore_flash_node_init(void)
{
    int ret = 0;
    flash_kobj = kobject_create_and_add("flash", &tool_info.gcore_dev->kobj);
    if (!flash_kobj) {
        GTP_ERROR("failed create flash dir");
        return -EINVAL;
    }
    ret = sysfs_create_bin_file(flash_kobj, &flash_attr);
    if (ret) {
        GTP_ERROR("failed create flashdata bin node, %d", ret);
        kobject_put(flash_kobj);
    }
    GTP_DEBUG("creat flashdata successful");
    return ret;
}

void gcore_flash_node_deinit(void)
{
    if (!flash_kobj)
        return;
    sysfs_remove_bin_file(flash_kobj, &flash_attr);
    kobject_put(flash_kobj);
}
#endif
