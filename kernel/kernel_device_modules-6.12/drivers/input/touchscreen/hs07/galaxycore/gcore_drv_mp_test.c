...
/*HS07 code for HS07-245 by huangyin at 20250401 start*/
#define FW_BUFF_EXT_LEN 3
/*HS07 code for HS07-245 by huangyin at 20250401 end*/
...
int gcore_parse_mp_test_ini(struct gcore_mp_data *mp_data)
{
    ...
    if (ret == 0) {
        ...
        /*HS07 code for HS07-245 by huangyin at 20250401 start*/
        buff = kzalloc(fw->size + FW_BUFF_EXT_LEN, GFP_KERNEL);
        /*HS07 code for HS07-245 by huangyin at 20250401 end*/
...
int gcore_mp_test_item_open(struct gcore_mp_data *mp_data)
{
    ...
    if (ret) {
        ...
        //if (memcmp(cmd,read_cmd,sizeof(cmd)))
        {
            ...
            GTP_DEBUG("read cmd: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x",
...
int gcore_mp_bin_update(void)
{
...
#endif
#if GCORE_MP_TEST_ON
    fn_data.gdev->mp_ver_in_bin[0] = fw_buf[FW_VERSION_ADDR + 1];
    fn_data.gdev->mp_ver_in_bin[1] = fw_buf[FW_VERSION_ADDR];
    fn_data.gdev->mp_ver_in_bin[2] = fw_buf[FW_VERSION_ADDR + 3];
    fn_data.gdev->mp_ver_in_bin[3] = fw_buf[FW_VERSION_ADDR + 2];
#endif
    ...
    mutex_lock(&g_mp_data->gdev->fwupdate_lock);
...
#endif
    mutex_unlock(&g_mp_data->gdev->fwupdate_lock);
    return 0;
