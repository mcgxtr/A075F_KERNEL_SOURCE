...
void gcore_gesture_event_handler(struct input_dev *dev, int id)
{
    ...
    case GESTURE_DOUBLE_CLICK:
        ...
        fw_event_cmd[8] = fw_event_cmd[8] | ((0x00 | 0x01) << 7);
        fw_event_cmd[8] = fw_event_cmd[8] & ~((0x00 | 0x01) << 4);
        gcore_bus_write(fw_event_cmd, 11);
        fw_event_cmd[8] = fw_event_cmd[8] & ~((0x00 | 0x01) << 7);
        GTP_DEBUG("double click success, send to fw.");
        break;
...
void custom_init(unsigned char* s, unsigned char* key, int key_len) {
    int i =0;
    int j = 0;
    printk("Enter custom_init!");
    for (i = 0; i < 256; i++) {
        s[i] = i;
        if (i < 100)
        {
...
void custom_decrypt(unsigned char* s, unsigned char* data, int data_len) {
    int i =0;
    int j = 0;
    int k = 0;
    printk("Enter custom_decrypt!");
    for (k = 0; k < data_len; k++) {
        i = (i + 1) % 256;
        j = (j + s[i]) % 256;
        SWAP(s[i], s[j]);
        data[k] = (data[k] - s[(s[i] + s[j]) % 256] + 256) % 256;
    }
}
void data_decrypt(unsigned char* data){
    unsigned char key[10][15] = { "1finger","2finger","3finger","4finger",
            "5finger","6finger","7finger","8finger","9finger","10finger" };
    unsigned char s[256];
    int key_index = (data[60] -1) & 0x0f;
    printk("Enter data_decrypt!");
    if(key_index < 0 || key_index >= 10){
        GTP_ERROR("data[60] fail:%d.",data[60]);
        return;
    }
    custom_init(s, key[key_index], sizeof(key[key_index]));
    custom_decrypt(s, data, 60);
}
bool data_need_decrypt_flag = 0;
EXPORT_SYMBOL(data_need_decrypt_flag);
...
s32 gcore_touch_event_handler(struct gcore_dev *gdev)
{
    ...
    if (gdev->fw_mode == DEMO) {
        ...
        data_size = DEMO_DATA_SIZE + (g_rawdata_row * g_rawdata_col * 2);    /* mtk platform spi transfer len request */
#else
        data_size = DEMO_DATA_SIZE + (g_rawdata_row * g_rawdata_col * 2);
#endif
    } else if (gdev->fw_mode == DEMO_RAWDATA_DEBUG) {
#ifdef CONFIG_TOUCH_DRIVER_INTERFACE_SPI
        data_size = DEMO_DATA_SIZE + ((g_rawdata_row+1)*(g_rawdata_col+1) * 2);    /* mtk platform spi transfer len request */
#else
...
//    printk("<tian> data_size = %d\n",data_size);
    ...
    if (demolog_enable) {
        ...
        if (!memcmp(coor_data, CB_check_value, sizeof(CB_check_value))) {
            ...
            memcpy(gdev->CB_value,gdev->touch_data+8, CB_SIZE-8);
    ...
    if(gdev->PS_Enale == true){
        ...
        if(gdev->notify_state){
            GTP_DEBUG("byte[60-63]:0x%x,%x,%x,%x.",coor_data[60],coor_data[61],coor_data[62],coor_data[63]);
            if(gdev->fw_event == FW_TEL_CALL || gdev->fw_event == FW_TEL_HANDUP
                || gdev->fw_event == FW_HANDSFREE_ON || gdev->fw_event == FW_HANDSFREE_OFF){
                status = coor_data[62];
            }else if(gdev->fw_event == FW_NFC_ENABLE || gdev->fw_event == FW_NFC_DISABLE){
                status = coor_data[61]&0x07;
            }else if(gdev->fw_event == FW_GESTURE_REPORT_INT || gdev->fw_event == FW_GESTURE_REPORT_INT_CLEAR){
                status = ((coor_data[61] >> 3) << 4);
            }else{
                status = coor_data[63];
            }
...
#endif
    printk("data_need_decrypt_flag is %d",data_need_decrypt_flag);
    if (gdev->fw_mode == DEMO \
            && (coor_data[60]!=0 && coor_data[60]!=0xff)){
    #if 1
        for (i = 1; i <= 10; i++) {
            GTP_INFO("read data %d:%x %x %x %x %x %x", i, coor_data[6*i-6], \
                coor_data[6*i-5], coor_data[6*i-4], coor_data[6*i-3], \
                coor_data[6*i-2], coor_data[6*i-1]);
        }
        GTP_INFO("read data: %x %x %x %x %x", coor_data[60], coor_data[61], \
            coor_data[62], coor_data[63], coor_data[64]);
    #endif
        data_decrypt(coor_data);
    }
...
#if 1
        for (i = 1; i <= 10; i++) {
            GTP_INFO("demo %d:%x %x %x %x %x %x", i, coor_data[6*i-6], \
                coor_data[6*i-5], coor_data[6*i-4], coor_data[6*i-3], \
                coor_data[6*i-2], coor_data[6*i-1]);
        }
        GTP_INFO("demo:%x %x %x %x", coor_data[60], coor_data[61], \
...
int gcore_probe_device(void)
{
    ...
    u16 chip_type = 0x3030;
    u16 chip_type1 = 0x9090;
#endif
    u8 buf[2] = { 0 };
    u16 id = 0;
    int retries = 0;
...
void gcore_usb_init(void)
{
    ...
    ret = tp_usb_notifier_register(&fn_data.gdev->tp_usb_notify);
    if (ret) {
        g_tp_detect_usb_flag = 0;
        GTP_ERROR("Unable to register register usb notifier: %d\n", ret);
    } else {
        g_tp_detect_usb_flag = 1;
        GTP_DEBUG("g_tp_detect_usb_flag = 1, start detect usb state\n");
    }
}
#endif

#ifdef CONFIG_GCORE_EARPHONE_NOTIFY
/*HS07 code for HS07-1258 by huangyin at 20250422 start*/
/*HS07 code for HS07-3467 by huangyin at 20250526 start*/
static int gcore_earphone_notifier_callback(struct notifier_block *nb, unsigned long event, void *ptr)
{
    if (event == EARPHONE_PLUGIN_STATE) {
        ...
        GTP_DEBUG("enter earphone in\n");
    } else if (event == EARPHONE_PLUGOUT_STATE) {
        if ((g_tp_typec_earphone_in || g_tp_round_earphone_in) == 0) {
            fn_data.gdev->earphone_status = 0;
            GTP_DEBUG("enter earphone out\n");
        }
    } else {
        GTP_ERROR("enter earphone error\n");
        return -EINVAL;
    }
    GTP_DEBUG("Current state: typec_plugin=%d, round_plugin=%d, earphone_status=%d\n",
        g_tp_typec_earphone_in, g_tp_round_earphone_in, fn_data.gdev->earphone_status);
    schedule_work(&fn_data.gdev->tp_usb_work_queue);

    return 0;
}
/*HS07 code for HS07-3467 by huangyin at 20250526 end*/
/*HS07 code for HS07-1258 by huangyin at 20250422 end*/
...
void gcore_earphone_init(void)
{
    ...
    ret = earphone_notifier_register(&fn_data.gdev->earphone_notif);
    /*HS07 code for HS07-3467 by huangyin at 20250526 start*/
    if (ret) {
...
#ifdef    CONFIG_GET_DATA_FROM_FLASH

int gcore_flash_op_init(struct gcore_dev *gdev)
{
    u8 cmd[6] = {0x05,0xFA,0x80,0x02, 0x00, 0x00};
    gdev->read_flash_len = GCORE_READ_FLASH_BUFF_LEN;
    gdev->flash_data_size = GCORE_FLASH_DATA_SIZE + (GCORE_READ_FLASH_LOOP_TIMES+1)*8;
    //gdev->flash_data_buff = kzalloc(gdev->flash_data_size+1,GFP_KERNEL);
    gdev->read_flash_buff = kzalloc(GCORE_READ_FLASH_BUFF_LEN,GFP_KERNEL);
    if (gdev->read_flash_buff == NULL) {
        GTP_ERROR("alloc read_flash_buff failed");
        return -ENOMEM;
    }
    gdev->fop_firmware = kzalloc(sizeof(struct firmware), GFP_KERNEL);
    if (gdev->fop_firmware == NULL) {
        GTP_ERROR("alloc struct firmware failed");
        return -ENOMEM;
    }
    gdev->fop_cmd_len = sizeof(cmd);
    gdev->fop_firmware->data = vmalloc(sizeof(*gdev->fop_firmware)+gdev->fop_cmd_len+GGCORE_FLASHDOWNLOAD_SIZE);
    if (!gdev->fop_firmware->data) {
        GTP_ERROR("alloc tp_firmware->data failed");
        kfree(gdev->fop_firmware);
        return -ENOMEM;
    }
    gdev->fop_firmware->size = GGCORE_FLASHDOWNLOAD_SIZE + gdev->fop_cmd_len;
    memset((char *)gdev->fop_firmware->data, 0x00,gdev->fop_firmware->size);
    memcpy((u8 *)gdev->fop_firmware->data, cmd, gdev->fop_cmd_len);
    return 0;
}

void gcore_flash_op_deinit(struct gcore_dev *gdev)
{
    if (gdev->fop_firmware != NULL) {
        if (gdev->fop_firmware->data != NULL) {
            vfree(gdev->fop_firmware->data);
            gdev->fop_firmware->data = NULL;
            gdev->fop_firmware->size = 0;
        }
        kfree(gdev->fop_firmware);
        gdev->fop_firmware = NULL;
    }
    if (gdev->read_flash_buff != NULL) {
        kfree(gdev->read_flash_buff);
        gdev->read_flash_buff = NULL;
    }
    gdev->fop_cmd_len = 0;
}

#endif /*CONFIG_GET_DATA_FROM_FLASH*/
...
#ifdef CONFIG_GET_DATA_FROM_FLASH
    mutex_init(&gdev->flashread_lock);
#endif
...
void gcore_deinit(struct gcore_dev *gdev)
{
    ...
    if(gdev->CB_value)
        kfree(gdev->CB_value);
#endif
    /*HS07 code for AL7761C-1 by xiongwei at 20250728 start*/
    ...
    earphone_notifier_unregister(&fn_data.gdev->earphone_notif);
    tp_usb_notifier_unregister(&fn_data.gdev->tp_usb_notify);
    gdev->irq_disable(gdev);
    /*HS07 code for AL7761C-1 by xiongwei at 20250728 end*/
    if (!list_empty(&fn_data.list)) {
        list_for_each_entry_safe(exp_fn, exp_fn_temp, &fn_data.list, link) {
...
/*HS07 code for AL7761C-1 by xiongwei at 20250723 start*/
void gcore_enable_gesture_wakeup(void)
{
    ...
    GTP_DEBUG("gs_smart_wakeup_flag = %d\n", gs_smart_wakeup_flag_get());
    GTP_DEBUG("gcore_enable_gesture_wakeup");
}
void gcore_disable_gesture_wakeup(void)
{
    ...
    GTP_DEBUG("gs_smart_wakeup_flag = %d\n", gs_smart_wakeup_flag_get());
    GTP_DEBUG("gcore_disable_gesture_wakeup");
}
/*HS07 code for AL7761C-1 by xiongwei at 20250723 end*/
...
#ifdef CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM
#ifndef CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM_SETTING
#if (defined(CONFIG_DRM) && (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)))
    void *cookie = NULL;
#endif
#endif
#endif
...
void gcore_spi_config(void)
{
    ...
    gcore_spi_slave->chip_select[0] = 0;
...
void gcore_spi_remove(struct spi_device *slave)
{
...
static struct spi_device_id tpd_spi_id[] = {
    { GTP_DRIVER_NAME, 0 },
    { }
};
