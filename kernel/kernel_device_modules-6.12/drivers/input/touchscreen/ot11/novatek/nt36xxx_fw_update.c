...
static int32_t nvt_read_ram_and_save_file(uint32_t addr, uint16_t len, char *name)
{
    ...
    set_fs(org_fs);
#else
    ret = kernel_write(fp, fbufp+1, len, &file_offset);
#endif
...
int32_t nvt_update_firmware(const char *firmware_name)
{
    ...
    input_info(true, &ts->client->dev, "%s: Update firmware success! <%ld us>\n",
            __func__, (long) ktime_us_delta(end, start));
...
int nvt_ts_fw_update_from_ums(struct nvt_ts_data *ts)
{
    ...
    if (fw_size > 0) {
        ...
        set_fs(org_fs);
#else
        nread = kernel_read(fp, fw_data, fw_size, &fp->f_pos);
#endif
...
void Boot_Update_Firmware(struct work_struct *work)
{
    ...
    if (ts->isResumeUpdate) {
        input_info(true, &ts->client->dev, "The firmware has been updated and there is no need to update it again!\n");

        /*Tab A9 code for AX6739A-1870 by yuli at 20230714 start*/
        nvt_set_usb_mode(ts->usb_plug_status);
        /*Tab A9 code for AX6739A-1870 by yuli at 20230714 end*/
