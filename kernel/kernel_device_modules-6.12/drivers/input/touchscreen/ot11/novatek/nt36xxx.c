...
static int nvt_parse_dt(struct device *dev)
{
   ...
   /*Tab A9 code for AX6739W-46 by xiongwei at 20250827 start*/
    if (!g_board_id_status) {
        #if defined(CONFIG_CUSTOM_PROJECT_OT11_WIFI_ALL) || defined(CONFIG_CUSTOM_PROJECT_HST11_WIFI)
    ...
    /*Tab A9 code for AX6739W-46 by xiongwei at 20250827 end*/
#endif
...
static int nvt_gpio_config(struct device *dev, struct nvt_ts_platdata *platdata)
{
    ...
    if ((dev == NULL) || (platdata == NULL)) {
        input_err(true, &ts->client->dev, "The parameter dev or platdata is NULL\n");
        return -ENODEV;
    }
#if NVT_TOUCH_SUPPORT_HW_RST
...
*/
    ...
    for (i = 0, finger_cnt = 0; i < platdata->max_touch_num; i++) {
        ...
        if ((status == FINGER_ENTER) || (status == FINGER_MOVING) || (status == GLOVE_TOUCH) || (status == PALM_TOUCH) || (status == METAL_JIG_TOUCH)) {
            ...
            /*Tab A9 code for AX6739W-46 by xiongwei at 20250616 start*/
            input_report_key(ts->input_dev, BTN_PALM, (palm | metal_jig));
            /*Tab A9 code for AX6739W-46 by xiongwei at 20250616 end*/
...
void nvt_enable_gesture_wakeup(void)
{
    if (!ts->platdata->enable_settings_aot) {
        input_err(true, &ts->client->dev, "aot feature not support %d\n",
            ts->platdata->enable_settings_aot);
        goto out;
    } else {
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
    ...
    if (client->dev.of_node) {
        ...
        if (ret < 0) {
            input_err(true, &client->dev, "Failed to parse dt(%d)\n", ret);
    ...
    /*Tab A9 code for AX6739A-1195 by yuli at 20230615 start*/
    input_info(true, &ts->client->dev, "mode=%d, max_speed_hz=%d, cs_id=%d\n", ts->client->mode, ts->client->max_speed_hz, ts->client->chip_select[0]);
    /*Tab A9 code for AX6739A-1195 by yuli at 20230615 end*/
/*
    //---parse dts---
    ret = nvt_parse_dt(&client->dev);
...
*/
    ...
    if (platdata == NULL) {
        input_err(true, &ts->client->dev, "The parameter platdata->irq_gpio is NULL\n");
