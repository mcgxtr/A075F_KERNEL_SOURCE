...
struct ic_setup_collect g_hx83102e_setup = {
    ...
    ._addr_hand_blade          = addr_hand_blade_addr,
    ._data_hand_blade          = data_hand_blade_on_off,
#endif    
};
...
static bool hx83102e_chip_detect(void)
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        && ((tmp_data[1] == 0x2e))) {
            ...
            if (hx83102e_register_read(HX83102G_REG_ICID_DOUBLE_CHK,
                tmp_data1, DATA_LEN_4) != 0) {
                E("%s:102_register_read Fail.\n", __func__);
                ret = false;
                goto FINAL;
            }
            if (tmp_data1[0] == 0x88) {
                is_hx83102e_flag = false;    
            } else {
                is_hx83102e_flag = true ;
            }
            strncpy(hx_s_ts->chip_name,
                HX_83102E_SERIES_PWON, 30);
            
            if ( is_hx83102e_flag == true ){
                (hx_s_ic_data)->ic_adc_num =
                    HX83102E_DATA_ADC_NUM;
            }else {
                (hx_s_ic_data)->ic_adc_num =
                    HX83102G_DATA_ADC_NUM;
            }
