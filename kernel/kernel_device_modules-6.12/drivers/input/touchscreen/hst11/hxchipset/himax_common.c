...
void himax_ts_work(struct himax_ts_data *ts)
{
    ...
    int ts_path = 0;
#if defined(CONFIG_PM) && HX_PATCH_COMERR_PM
    int ret = 0;
#endif
...
/*HST11 code for AX7800A-359 by yubo at 20250418 start*/
void mapping_panel_id_from_panel_name(void)
{
    int module = 0;
...
/*HST11 code for AX7800A-359 by yubo at 20250418 end*/
#endif
...
/* HST11 code for AX7800A-370 by suyurui at 20250418 start */
...
/* HST11 code for AX7800A-370 by suyurui at 20250418 end */
...
/*HST11 code for AX7800A-359 by yubo at 20250418 start*/
int himax_get_tp_module(void)
{
    int fw_num = 0;
    const char *panel_name = NULL;
    panel_name = tp_choose_panel();

    if (NULL != strstr(panel_name, "hx83102e_lc_hkc")) {
        fw_num = MODEL_LC_HKC;
    } else if (NULL != strstr(panel_name, "hx83102g_xy_inx")) {
        fw_num = MODEL_XY_INX;
    } else {
...
static void himax_update_module_info(void)
{
    ...
    ts->boot_fw_name = (char *)kzalloc(HIMAX_NAME_LEN, GFP_KERNEL);
    ts->mp_fw_name = (char *)kzalloc(HIMAX_NAME_LEN, GFP_KERNEL);
    switch (module)
    {
        case MODEL_LC_HKC:
                strcpy(ts->module_name,"hx83102e_lc_hkc");
                strcpy(ts->boot_fw_name, "hx83102e_lc_hkc_firmware.bin");
                strcpy(ts->mp_fw_name, "hx83102e_lc_hkc_mpfw.bin");
                strcpy(ts->himax_csv_name, "hx83102e_lc_hkc_criteria.csv");
                break;
        case MODEL_XY_INX:
                strcpy(ts->module_name,"hx83102g_xy_inx");
                strcpy(ts->boot_fw_name, "hx83102g_xy_inx_firmware.bin");
                strcpy(ts->mp_fw_name, "hx83102g_xy_inx_mpfw.bin");
                strcpy(ts->himax_csv_name, "hx83102g_xy_inx_criteria.csv");
                break;
        default:
                strcpy(ts->module_name,"unknown");
                strcpy(ts->boot_fw_name, "unknown.bin");
                strcpy(ts->mp_fw_name, "unknown.bin");
 ...
 */
    ...
    if (ret) {
        ...
        E("Unable to register earphone notifier: %d\n", ret);
        goto err_headset_notify_failed;
    } else {
        g_tp_detect_typec_flag = 1;
        I("Ready to register earphone notifier: %d\n", ret);
    }
    /* HST11 code for AX7800A-1018 by suyurui at 20250531 end */
    ...
    if (ret) {
        ...
        E("Unable to register usb notifier: %d\n", ret);
