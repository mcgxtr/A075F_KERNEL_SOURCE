...
/*Tab A11 code for AX7800W-11 by xiongwei at 20250708 start*/
#include "panel-bias-hst11.h"
/*Tab A11 code for AX7800W-11 by xiongwei at 20250708 end*/
...
/*Tab A11 code for AX7800W-11 by tangchuanjing at 20250901 start*/
extern const struct firmware *fw;
/*Tab A11 code for AX7800W-11 by tangchuanjing at 20250901 end*/
...
char *jd_i_CTPM_firmware_name = "jd9366tc_xy_csot_firmware.bin";
#endif
...
/*Tab A11 code for AX7800W-11 by tangchuanjing at 20250804 start*/
#if defined(JADARD_GKI)
/*Tab A11 code for AX7800W-11 by tangchuanjing at 20250804 end*/
...
#if defined(CONFIG_PM) && defined(JD_PATCH_COMERR_PM)
    int ret = 0;
#endif
...
/*HST11 code for AX7800A-2000 by huangpengyu at 20250606 start*/
            if (((rdata[1] == 0x00) && (rdata[2] == 0x00) && (rdata[3] == 0x00)) || (same_data >= 5)) {
                if ((rdata[0] == 0xDE) || (rdata[0] == 0xE0) || (rdata[0] == 0xE2) || (rdata[0] == 0xE4) ||
                    (same_data >= 5)) {
                    if (pjadard_ts_data->fw_ready == true) {
                        JD_I("PC(0-3):0x%02x, 0x%02x, 0x%02x, 0x%02x\n", rdata[0], rdata[1], rdata[2], rdata[3]);
/*HST11 code for AX7800A-2000 by huangpengyu at 20250606 end*/
...
static void jadard_enable_gesture_wakeup(void)
{
    ...
    if (ts != NULL) {
        ...
        JD_E("pjadard_ts_data is NULL,can not set gs_smart_wakeup_flag_get!");
    }

    //JD_I("%s ---,g_smart_wakeup_flag = %d\n",__func__,gs_smart_wakeup_flag_get());//bringup
}
...
static void tp_feature_interface(void)
{
    ...
    /*Tab A11 code for AX7800W-11 by tangchuanjing at 20250811 start*/
    gs_hx_lcm_info->module_name = ts->module_name;
    /*Tab A11 code for AX7800W-11 by tangchuanjing at 20250811 end*/
...
static int jadard_usb_notifier_callback(struct notifier_block *nb, unsigned long event, void *ptr)
{
    ...
    if (ts->suspended == false && ts->fw_ready == true) {
	/*Tab A11 code for AX7800W-11 by tangchuanjing at 20250709 start*/
        JD_I("%s notify_event=%lu, gs_jd_usb_state = %d\n", __func__, event, gs_jd_usb_state[1]);
	/*Tab A11 code for AX7800W-11 by tangchuanjing at 20250709 end*/
...
static int jadard_earphone_notifier_callback(struct notifier_block *nb, unsigned long event, void *ptr)
{
    ...
    /* HST11 code for AX7800A-1018 by suyurui at 20250531 start */
    if (event == EARPHONE_PLUGIN_STATE) {
        g_jd_earphone_state = 2;
        JD_I("enter earphone plugin!\n");
    } else if(event == EARPHONE_PLUGOUT_STATE) {
        if ((g_tp_typec_earphone_in || g_tp_round_earphone_in) == 0) {
            g_jd_earphone_state = 1;
            JD_I("enter earphone plugout!\n");
        }
    } else {
        JD_I("%s event value is fail\n", __func__);
        return -EINVAL;
    ...
    if (ts->suspended == false && ts->fw_ready == true) {
	/*Tab A11 code for AX7800W-11 by tangchuanjing at 20250709 start*/
        JD_I("%s notify_event=%lu, typec_plugin=%d, round_plugin=%d,g_jd_earphone_state = %d\n",
        __func__, event,g_tp_typec_earphone_in, g_tp_round_earphone_in,  g_jd_earphone_state);
        schedule_work(&ts->earphone_work_queue);
	/*Tab A11 code for AX7800W-11 by tangchuanjing at 20250709 end*/
    /* HST11 code for AX7800A-1018 by suyurui at 20250531 end */
    } else {
        JD_I("%s:is suspended, do nothing", __func__);
    }
    JD_I("%s ---\n", __func__);
...
/*Tab A11 code for AX7800W-11 by tangchuanjing at 20250903 start*/
int jadard_get_tp_module(void)
{
    int fw_num = 0;
    const char *panel_name = NULL;
    panel_name = tp_choose_panel();
    if (NULL != strstr(panel_name, "jd9366tc_xy_csot")) {
        fw_num = MODEL_XX_CSOT;
    } else if (NULL != strstr(panel_name, "jd9366tc_lx_hkc")) {
        fw_num = MODEL_LX_HKC;
    } else if (NULL != strstr(panel_name, "jd9366tc_ld_hkc")) {
        fw_num = MODEL_LD_HKC;
    } else {
...
static void jadard_update_module_info(void)
{
    int module = 0;
    struct jadard_ts_data *ts = pjadard_ts_data;
    module = jadard_get_tp_module();
    ts->jd_fw_rq_name = (char *)kzalloc(JD_NAME_LEN, GFP_KERNEL);
    switch (module)
    {
        case MODEL_XX_CSOT:
                strcpy(ts->module_name,"jd9366tc_xy_csot");
                strcpy(ts->jd_fw_rq_name, "jd9366tc_xy_csot_firmware.bin");
                strcpy(ts->jd_csv_name, "jd9366tc_xy_csot_ito.txt");
                break;
        case MODEL_LX_HKC:
                strcpy(ts->module_name,"jd9366tc_lx_hkc");
                strcpy(ts->jd_fw_rq_name, "jd9366tc_lx_hkc_firmware.bin");
                strcpy(ts->jd_csv_name, "jd9366tc_lx_hkc_ito.txt");
                break;
        case MODEL_LD_HKC:
                strcpy(ts->module_name,"jd9366tc_ld_hkc");
                strcpy(ts->jd_fw_rq_name, "jd9366tc_ld_hkc_firmware.bin");
                strcpy(ts->jd_csv_name, "jd9366tc_ld_hkc_ito.txt");
                break;
        default:
                strcpy(ts->module_name,"unknown");
                strcpy(ts->jd_fw_rq_name, "unknown.bin");
                strcpy(ts->jd_csv_name, "unknown.csv");
                break;
    }

    jd_i_CTPM_firmware_name = ts->jd_fw_rq_name;
}
/*Tab A11 code for AX7800W-11 by tangchuanjing at 20250903 end*/
...
if (err) {
    ...
    if (err) {
        ...
        JD_I("Ready to register earphone notifier: %d\n", err);
    }
    /* HST11 code for AX7800A-1018 by suyurui at 20250531 end */
#endif //JD_EARPHONE_DETECT_NOTIFY
/*HST11 code for AX7800A-354 by huangpengyu at 20250421 end*/
...
/*HST11 code for AX7800A-437 by huangpengyu at 20250428 start*/
#ifdef JD_ZERO_FLASH
        /* Reduce resume time when sleep in IOVCC drop */
        JD_I("%s: Not check pram crc\n", __func__);
        ts->power_on_upgrade = false;
#endif
/*HST11 code for AX7800A-437 by huangpengyu at 20250428 end*/
    }
#endif
    ...
    if (pjadard_ic_data != NULL) {
        JD_I("IC_NAME:JD9366TC_XY_CSOT TOUCH_VER:%02x\n", pjadard_ic_data->fw_cid_ver & 0xFF);
    }
