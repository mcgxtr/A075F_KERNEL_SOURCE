...
static int jd9366t_GetID(uint16_t *pRomID)
{
    ...
    for (retry = 0; retry < MAX_RETRIES; retry++) {
        ...
        ReCode = jd9366t_ReadRegMulti((uint32_t)JD9366T_SOC_REG_ADDR_CHIP_ID2,
                                     rbuf, sizeof(rbuf));
        
        if (ReCode < 0) {
            last_read_error = ReCode;
            JD_E("%s: Read failed (retry %d/%d), code: %d\n",
                 __func__, retry+1, MAX_RETRIES, ReCode);
            continue;
        }
        ...
        *pRomID = (uint16_t)((rbuf[1] << 8) + rbuf[0]);
        if (*pRomID != JD9366T_ID) {
            id_error_occurred = 1;
            JD_E("%s: ID mismatch (retry %d/%d), got: 0x%04X\n",
                 __func__, retry+1, MAX_RETRIES, *pRomID);
            continue;
        }

        JD_I("%s: Valid IC ID: 0x%04X (retry %d/%d)\n",
             __func__, *pRomID, retry+1, MAX_RETRIES);
        return ReCode;
    }

    if (id_error_occurred) {
        JD_E("%s: Final error: IC ID mismatch after %d retries\n",
             __func__, MAX_RETRIES);
        return JD_CHIP_ID_ERROR;
    } else {
        JD_E("%s: Final error: Read failed after %d retries, last code: %d\n",
             __func__, MAX_RETRIES, last_read_error);
    ...
    if (jd9366t_GetSectionFirstValueStatus(50) == true) {
        ...
        if (ReCode < 0) {
            ...
            g_jd9366t_chip_info.dsram_fw_cfg_info_addr.fw_version =
                dsram_info_content[JD9366T_DSRAM_FW_CFG_INFO].info_content_addr + 6;
...
static int jd9366t_GetDMAStatus(uint16_t usTimeOut)
{
    ...
    do {
        ...
        mdelay(2);
        ReCode = jd9366t_ReadRegSingle((uint32_t)JD_FLASH_REG_ADDR_DMA_BUSY_OR_START, &usValue);
        ...
        dma_busy_status_done = g_module_fp.fp_DoneStatusIsLow(usValue, (uint8_t)JD9366T_DMA_RELATED_SETTING_DMA_BUSY,
...
static int jd9366t_GetPageProgramStatus(uint16_t usTimeOut)
{
    ...
    do {
        ...
        mdelay(2);
        ReCode = jd9366t_ReadRegSingle((uint32_t)JD_FLASH_REG_ADDR_DMA_2WFLASHEN_1WRITE_0START, &usValue);
        ...
        dma_page_program_status_done = g_module_fp.fp_DoneStatusIsLow(usValue, (uint8_t)JD9366T_DMA_RELATED_SETTING_PAGE_PROGRAM_FLASH,
...
static int jd9366t_SetDMAStart(uint32_t flash_addr, uint32_t dma_size, uint32_t target_addr, uint8_t dma_cmd)
{
    ...
    ReCode = jd9366t_WriteRegMulti((uint32_t)JD_FLASH_REG_ADDR_DMA_FLASH_ADDR0, wBuf, sizeof(wBuf));
    if (ReCode < 0) {
...
static int jd9366t_SetDMAByteMode(void)
{
    ...
    return jd9366t_WriteRegMulti((uint32_t)JD_FLASH_REG_ADDR_DMA_BYTE_MODE, wBuf, sizeof(wBuf));
}
...
static uint8_t jd9366t_GetMoveInfoNumber(uint8_t *pFileData)
{
    if (pFileData[JD9366T_HW_HEADER_LEN] == 0xFF) {
        ...
        return (uint8_t)((pFileData[JD9366T_HW_HEADER_LEN] - JD9366T_HW_HEADER_INITIAL_NUMBER) /
...
static int jd9366t_ReadFlash(uint32_t addr, uint8_t *rdata, uint32_t rlen)
{
    ...
    uint16_t packlen = 0;
    /* MCU aram config */
    uint16_t ramsize = JD9366T_MEMORY_ARAM_L_SIZE;
    uint32_t ramaddr = 0;
    uint16_t ramlen = 0;
    /* DMA config */
...
static int jd9366t_HostReadFlash(uint32_t ReadAddr, uint8_t *pDataBuffer, uint32_t ReadLen)
{
    ...
    jd9366t_GetDataCRC16(pDataBuffer, ReadLen, 0, ReadLen, &softcrc, JD_CRC_INFO_PolynomialCRC16);
...
static int jd9366t_HostWriteFlash(uint32_t WriteAddr, uint8_t *pFileData, uint32_t FileSize)
{
    ...
    do {
        ...
        ReCode = g_module_fp.fp_SetFlashSPISpeed(pFileData[JD9366T_LOAD_CODE_SPI_FREQ]);
        if (ReCode < 0) {
            return ReCode;
        }
        ...
        if (FileSize >= WriteAddr + WriteLen) {
            ...
            jd9366t_GetDataCRC16(crcbuffer, WriteLen, 0, WriteLen, &softcrc, JD_CRC_INFO_PolynomialCRC16);
            kfree(crcbuffer);
...
static int jd9366t_HostWritePram(uint32_t WriteAddr, uint8_t *pFileData, uint32_t FileSize)
{
    ...
    for (i = 0; i < MoveInfoNumber; i++) {
        ...
        softcrc = JD_CRC_INFO_CRC_INITIAL_VALUE;
        jd9366t_GetDataCRC16(pFileData + MoveInfo[i].fl_st_addr, MoveInfo[i].fl_len, 0, MoveInfo[i].fl_len, &softcrc, JD_CRC_INFO_PolynomialCRC16);
...
static int jd9366t_HostReadPram(uint32_t ReadAddr, uint8_t *pFileData, uint32_t ReadLen)
{
    ...
    softcrc = JD_CRC_INFO_CRC_INITIAL_VALUE;
    jd9366t_GetDataCRC16(pFileData, ReadLen, ReadAddr, ReadLen, &softcrc, JD_CRC_INFO_PolynomialCRC16);
...
static uint8_t module_jd9366t_DbicReg_write(uint8_t page, uint8_t cmd, uint8_t *par, uint8_t par_len, uint32_t offset)
{
    uint8_t i;
...
static void module_jd9366t_read_fw_ver(void)
{
    ...
    if ((ReCode_cid < 0) || (ReCode_ver < 0) || (ReCode_maker < 0) || (ReCode_panel < 0)) {
        ...
        pjadard_ic_data->fw_ver = ((rdata_fw[0] << 24) |
                                   (rdata_fw[1] << 16) |
                                   (rdata_fw[2] << 8) |
                                   (rdata_fw[3]));
...
static void module_jd9366t_set_earphone_enable(uint8_t status)
{
    uint8_t wdata[JD_TWO_SIZE] = {0x00, 0x00};
