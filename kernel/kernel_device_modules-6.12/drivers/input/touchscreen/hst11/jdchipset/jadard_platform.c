...
/*Tab A11 code for AX7800W-11 by tangchuanjing at 20250708 start*/
/*
static struct mtk_chip_config spi_ctrdata = {
    .cs_setuptime = JD_CS_SETUPTIME,   //1us == 100*10 ns for jd9366tc
};
*/
/*Tab A11 code for AX7800W-11 by tangchuanjing at 20250708 end*/
...
static int jadard_common_pm_suspend(struct device *dev)
{
    struct jadard_ts_data *ts = dev_get_drvdata(dev);

    JD_D("%s system enters into pm_suspend\n", __func__);
    ts->pm_suspend = true;
...
static int jadard_common_pm_resume(struct device *dev)
{
    struct jadard_ts_data *ts = dev_get_drvdata(dev);

    JD_D("%s system resumes from pm_suspend\n", __func__);
    ts->pm_suspend = false;
...
#if (!defined(JD_CONFIG_FB)) && (!defined(JD_CONFIG_DRM))
    .suspend = jadard_common_pm_suspend,
    .resume  = jadard_common_pm_resume,
#endif
};
#endif
