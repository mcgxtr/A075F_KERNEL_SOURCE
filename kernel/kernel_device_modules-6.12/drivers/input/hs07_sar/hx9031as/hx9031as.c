...
/* hs07 code for HS07-1446 by liuyiying at 20250428 start */
static uint8_t bl_int_num = 0;
/* hs07 code for HS07-1446 by liuyiying at 20250428 end */
...
/* hs07 code for HS07-359 by huofudong at 20250407 start */
/* hs07 code for HS07-870 by liuyiying at 20250417 start */
/* hs07 code for HS07-2625 | HS07-2808 by huofudong at 20250510 start */
/* hs07 code for HS07-4274 by liuyiying at 20250609 start */
static struct hx9031as_near_far_threshold hx9031as_ch_thres[HX9031AS_CH_NUM] = {
    {.thr_near = 32000, .thr_far = 32000}, //ch0
    {.thr_near = 224, .thr_far = 224},
    {.thr_near = 192, .thr_far = 192},
    {.thr_near = 192, .thr_far = 192},
    {.thr_near = 896, .thr_far = 864},
};
/* hs07 code for HS07-4274 by liuyiying at 20250609 end */
/* hs07 code for HS07-2625 | HS07-2808 by huofudong at 20250510 end */
/* hs07 code for HS07-870 by liuyiying at 20250417 end */
/* hs07 code for HS07-359 by huofudong at 20250407 end */
...
static void hx9031as_ch_cfg(void)
{
    ...
    /* hs07 code for HS07-359 by huofudong at 20250407 start */
    /* hs07 code for HS07-1436 by liuyiying at 20250423 start */
    CH0_POS = CS1;
    CH0_NEG = NA;
    CH1_POS = CS0;
    CH1_NEG = CS1;
    ...
    CH4_POS = CS2;
    CH4_NEG = NA;
    /* hs07 code for HS07-1436 by liuyiying at 20250423 end */
    /* hs07 code for HS07-359 by huofudong at 20250407 end */
...
/* hs07 code for HS07-1446 by liuyiying at 20250428 start */
static void hx9031as_offset_abnormal_process(void)
{
    int ret = -1;
    int ii = 0;
    uint8_t buf[1] = {0};
    uint8_t val_0x24[1] = {0};
    uint8_t val_0x35[1] = {0};
    uint8_t val_0x3b[1] = {0};
    uint8_t bl_abn_state = 0;

    ret = hx9031as_read(RW_24_CH_NUM_CFG, val_0x24, 1);
    ret |= hx9031as_read(RW_35_FORCE_NO_UP_CFG, val_0x35, 1);
    ret |= hx9031as_read(RW_3B_CALI_DIFF_CFG, val_0x3b, 1);

    if (0 != ret) {
        PRINT_ERR("hx9031as_read failed\n");
    }
    for (ii = 0; ii < HX9031AS_CH_NUM; ii++) {
        if (((val_0x24[0] >> ii) & 0x01) && (data_offset_dac[ii] == 0))
            bl_abn_state |= (0x01 << ii);
    }
    if (bl_abn_state != 0) {
        bl_int_num++;
        if (bl_int_num == 5) {
            bl_int_num = 0;
            buf[0] = val_0x35[0] & 0xF0;
            ret = hx9031as_write(RW_35_FORCE_NO_UP_CFG, buf, 1);
            buf[0] = val_0x3b[0] & 0xEF;
            ret = hx9031as_write(RW_3B_CALI_DIFF_CFG, buf, 1);
            PRINT_INF("hx9031as_reset offset\n");
        }
    } else {
        bl_int_num = 0;
    }
    PRINT_INF("bl_abn_state = 0x%02X, bl_int_num=%d\n", bl_abn_state, bl_int_num);
}
/* hs07 code for HS07-1446 by liuyiying at 20250428 end */
#if 0
...
static void tyhx_anfr_irq(void)
{
    ...
    touch_state = (hx9031as_pdata.prox_state_reg >> 4) & 0x1;
...
/* hs07 code for HS07-359 by huofudong at 20250407 end */
/* hs07 code for SR-AL7761A-01-132 | SR-AL7761A-01-120 by huofudong at 20250320 end*/
...
static ssize_t hx9031_get_dumpinfo(char *buf)
{
    ...
    if(g_channel1 == CH1 && data_offset_dac[CH1] != 0) {
        total_cap = ((data_offset_dac[CH1] + data_offset_dac[CH0]) / 4 * 234) / 1000;
    } else {
...
#endif
    /* hs07 code for HS07-1446 by liuyiying at 20250428 start */
    hx9031as_offset_abnormal_process();
    /* hs07 code for HS07-1446 by liuyiying at 20250428 end */
    mutex_unlock(&hx9031as_ch_en_mutex);
    return IRQ_HANDLED;
}
...
/*Tab A11 code for HS07-3233  by lichang at 20250524 start*/
...
/*Tab A11 code for HS07-3233  by lichang at 20250524 end*/
