...
MODULE_PARM_DESC(silfp_ree_bufsiz,
		 "data bytes in biggest supported SILFP REE SPI message");
...
static int silfp_ree_parse_spi_dts(struct silfp_ree_data *ree_data)
{
	...
	do {
		node =
		    of_find_compatible_node(NULL, NULL, "mediatek,spidev-pins");
		if (!node) {
	...
	ree_data->pins_default =
	    pinctrl_lookup_state(ree_data->pinctrl_gpios, "default");
...
static ssize_t
silfp_ree_read(struct file *filp, char __user *buf, size_t count,
	       loff_t *f_pos)
{
			     ...
			     struct spi_ioc_transfer *u_xfers, unsigned n_xfers)
{
	...
	for (n = n_xfers, k_tmp = k_xfers, u_tmp = u_xfers;
		...
		LOG_MSG_DEBUG(INFO_LOG, &ree_data->spi->dev,
			      ...
			      u_tmp->tx_buf ? "tx " : "",
			      u_tmp->cs_change ? "cs " : "",
			      u_tmp->bits_per_word ? : ree_data->spi->
			      bits_per_word, u_tmp->delay_usecs,
...
static struct spi_ioc_transfer *silfp_ree_get_ioc_message(unsigned int cmd,
							  struct
							  spi_ioc_transfer
							  __user *u_ioc,
							  unsigned *n_ioc)
...
void silfp_spi_clk_enable(bool bonoff)
{
	...
	if (NULL == m_ree_data->spi) {
		LOG_MSG_DEBUG(INFO_LOG,
			      "%s, spi is null, enable or disable clk failed.",
			      __func__);
		return;
	}
	if (bonoff && (count == 0)) {
...
static int silfp_ree_probe(struct spi_device *spi)
{
	...
	if (spi->dev.of_node && !of_match_device(silfp_ree_dt_ids, &spi->dev)) {
		dev_err(&spi->dev,
			"buggy DT: ree_data listed directly in DT\n");
	...
	m_ree_device =
	    device_create(silfp_ree_class, NULL, MKDEV(silfp_ree_major, 0),
			  NULL, "silead_spi");
...
static void silfp_ree_remove(struct spi_device *spi)
{
...
static int __init silfp_ree_init(void)
{
	...
	if (silfp_ree_major < 0) {
		LOG_MSG_DEBUG(ERR_LOG, "register failed, silfp_ree_major = %d",
			      silfp_ree_major);
	...
	if (IS_ERR(silfp_ree_class)) {
		unregister_chrdev(silfp_ree_major,
				  silfp_ree_driver.driver.name);
