...
/*HS07 code for SR-AL7761A-01-523 by yuli at 20250317 start*/
extern int finger_sysfs;
/*HS07 code for SR-AL7761A-01-523 by yuli at 20250317 end*/
...
/* A07 code for AL7761C-1 by dingying at 20250722 start */
...
static void silfp_hw_poweron(struct silfp_data *fp_dev)
{
	...
	if (fp_dev->pin.pins_avdd_h) {
		err =
		    pinctrl_select_state(fp_dev->pin.pinctrl,
					 fp_dev->pin.pins_avdd_h);
	}
	if (fp_dev->pin.pins_vddio_h) {
		err =
		    pinctrl_select_state(fp_dev->pin.pinctrl,
					 fp_dev->pin.pins_vddio_h);
	}

	/*HS07 code for HS07-908 by yuli at 20250418 start */
	if (fp_dev->pin.pins_rst_h) {
		mdelay(5);
		pinctrl_select_state(fp_dev->pin.pinctrl,
				     fp_dev->pin.pins_rst_h);
	}
	/*HS07 code for HS07-908 by yuli at 20250418 end */

	if (fp_dev->pin.fp_spi_mode) {
		mdelay(1);
		pinctrl_select_state(fp_dev->pin.pinctrl,
				     fp_dev->pin.fp_spi_mode);
	}
...
static void silfp_hw_poweroff(struct silfp_data *fp_dev)
{
	...
	if (fp_dev->pin.pins_rst_l) {
		pinctrl_select_state(fp_dev->pin.pinctrl,
				     fp_dev->pin.pins_rst_l);
		mdelay(1);
	}

	if (fp_dev->pin.fp_gpio_mode) {
		pinctrl_select_state(fp_dev->pin.pinctrl,
				     fp_dev->pin.fp_gpio_mode);
		mdelay(1);
	}

	if (fp_dev->pin.pins_avdd_l) {
		pinctrl_select_state(fp_dev->pin.pinctrl,
				     fp_dev->pin.pins_avdd_l);
	}
	/* Power control by GPIOs */
...
static int silfp_parse_dts(struct silfp_data *fp_dev)
{
	...
	do {
		...
		if (!node) {
			LOG_MSG_DEBUG(ERR_LOG,
				      "%s compatible device node is null\n",
				      FP_IRQ_OF);
			ret = -1;
			break;
		}
	...
	fp_dev->pin.pins_irq =
	    pinctrl_lookup_state(fp_dev->pin.pinctrl, "irq-init");
	...
	fp_dev->pin.pins_rst_h =
	    pinctrl_lookup_state(fp_dev->pin.pinctrl, "rst-high");
	...
	fp_dev->pin.pins_rst_l =
	    pinctrl_lookup_state(fp_dev->pin.pinctrl, "rst-low");
	...
	fp_dev->pin.fp_spi_mode =
	    pinctrl_lookup_state(fp_dev->pin.pinctrl, "fp_spi_mode");
	...
	fp_dev->pin.fp_gpio_mode =
	    pinctrl_lookup_state(fp_dev->pin.pinctrl, "fp_gpio_mode");
...
#ifdef BSP_SIL_POWER_SUPPLY_PINCTRL
	fp_dev->pin.pins_avdd_h =
	    pinctrl_lookup_state(fp_dev->pin.pinctrl, "avdd-enable");
	...
	fp_dev->pin.pins_avdd_l =
	    pinctrl_lookup_state(fp_dev->pin.pinctrl, "avdd-disable");
	...
	fp_dev->pin.pins_vddio_h =
	    pinctrl_lookup_state(fp_dev->pin.pinctrl, "vddio-enable");
	...
	if (fp_dev->spi->dev.of_node) {
		/* Get the SPI ID (#1-6) */
		ret =
		    of_property_read_u32(fp_dev->spi->dev.of_node, "spi-id",
					 &fp_dev->pin.spi_id);
		if (ret) {
			fp_dev->pin.spi_id = 0;
		...
		ret =
		    of_property_read_u32(fp_dev->spi->dev.of_node, "spi-irq",
					 &fp_dev->pin.spi_irq);
		if (ret) {
		...
		ret =
		    of_property_read_u32(fp_dev->spi->dev.of_node, "spi-reg",
					 &fp_dev->pin.spi_reg);
		if (ret) {
...
static int silfp_set_spi(struct silfp_data *fp_dev, bool enable)
{
	...
	ms = spi_controller_get_devdata(fp_dev->spi->controller);
...
static int silfp_irq_to_reset_init(struct silfp_data *fp_dev)
{
	...
	fp_dev->pin.pins_irq_rst_h =
	    pinctrl_lookup_state(fp_dev->pin.pinctrl, "irq_rst-high");
	...
	fp_dev->pin.pins_irq_rst_l =
	    pinctrl_lookup_state(fp_dev->pin.pinctrl, "irq_rst-low");
			       ...
			       struct fp_dev_init_t *dev_info)
{
	...
	ret = request_irq(fp_dev->irq, silfp_irq_handler, IRQ_TYPE_EDGE_RISING,	//IRQ_TYPE_LEVEL_HIGH, //irq_table[ts->int_trigger_type],
			  "silfp", fp_dev);
	if (ret < 0) {
...
#endif
/* A07 code for AL7761C-1 by dingying at 20250722 end */
...
static void silfp_netlink_recv(struct sk_buff *__skb)
{
	...
	if (skb->len >= NLMSG_SPACE(0)) {
		...
		if ((strlen(str) == strlen("FP"))
		    && !strncmp(str, "FP", strlen(str))) {
			pid = nlh->nlmsg_pid;
...
static int silfp_netlink_init(struct silfp_data *fp_dev)
{
	...
	fp_dev->nl_sk =
	    netlink_kernel_create(&init_net, SIFP_NETLINK_ROUTE, &cfg);
	if (!fp_dev->nl_sk) {
			  ...
			  loff_t *ptr)
{
	...
	while (list_empty(&fp_dev->msg_q)) {
		...
		if (wait_event_interruptible
		    (fp_dev->read_queue, !list_empty(&fp_dev->msg_q)) < 0) {
			return -EINVAL;
		}
		spin_lock_irqsave(&fp_dev->read_lock, flags);
	}
	/* pick the first one */
	list = list_first_entry(&fp_dev->msg_q, struct silfp_msg_list, list);
	spin_unlock_irqrestore(&fp_dev->read_lock, flags);
...
{
	struct silfp_data *fp_dev =
	    container_of(notif, struct silfp_data, notif);
	...
	if (IS_KEY_VALID(pkey->value)) {
		...
		if (fp_dev->keymap_cust.k[pkey->value - NAV_KEY_START]) {
			LOG_MSG_DEBUG(INFO_LOG, "[%s] custom-key %d\n",
				      __func__,
				      fp_dev->keymap_cust.k[pkey->value -
							    NAV_KEY_START]);
			if (KEY_RESERVED !=
			    fp_dev->keymap_cust.k[pkey->value -
						  NAV_KEY_START]) {
...
#endif /* PROC_DIR */
		fp_dev->proc_entry =
		    proc_create(PROC_NODE, 0666, fp_dev->proc_root,
				&silfp_proc_fops);
		if (!fp_dev->proc_entry) {
			LOG_MSG_DEBUG(ERR_LOG,
				      "Create entry %s under /proc error!\n",
				      PROC_NODE);
			goto err_out1;
		}
	}
...
static int silfp_class_init(void)
{
	...
	ret =
	    sysfs_create_file(&(silfp_class_device->kobj),
...
static int silfp_input_init(struct silfp_data *fp_dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(fp_dev->keymap_cust.k); i++) {
		if (fp_dev->keymap_cust.k[i]
		    && (fp_dev->keymap_cust.k[i] != KEY_RESERVED)) {
			__set_bit(fp_dev->keymap_cust.k[i],
				  fp_dev->input->keybit);
		}
	}
...
static long silfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case SIFP_IOC_INIT:
		...
		if (err < 0) {
			LOG_MSG_DEBUG(INFO_LOG,
				      "[%s]silfp_resource_init fail\n",
				      __func__);
			retval = -EFAULT;
		}

		if (copy_to_user
		    ((void __user *)arg, (void *)&silfp_dev_init_d,
	...
	case SIFP_IOC_RESET:
		...
		if (arg) {
			if (copy_from_user
			    (&delay, (void __user *)arg, sizeof(char))) {
				retval = -EFAULT;
				break;
			}
		}
	...
	case SIFP_IOC_WAIT_IRQ:
		...
		retval =
		    __put_user(silfp_wait_irq(fp_dev) ? 1 : 0,
			       (__u8 __user *) arg);
		break;
	...
	case SIFP_IOC_IRQ_STATUS:
		...
		retval =
		    __put_user((char)silfp_irq_status(fp_dev),
			       (__u8 __user *) arg);
		break;

	case SIFP_IOC_KEY_EVENT:
		if (copy_from_user
		    (&key, (struct fp_dev_key_t *)arg,
	...
	case SIFP_IOC_SCR_STATUS:
		if (arg) {
			LOG_MSG_DEBUG(INFO_LOG, "[IOC_SCR_STATUS] put v = %d\n",
				      (u8) (!fp_dev->scr_off));
			retval =
			    __put_user((u8) (!fp_dev->scr_off),
				       (__u8 __user *) arg);
		} else {
			LOG_MSG_DEBUG(INFO_LOG,
				      "[IOC_SCR_STATUS] send v = %d\n",
				      (u8) (!fp_dev->scr_off));
			silfp_netlink_send(fp_dev,
					   fp_dev->
					   scr_off ? SIFP_NETLINK_SCR_OFF :
					   SIFP_NETLINK_SCR_ON);
		}
		break;
	...
	case SIFP_IOC_GET_VER:
		if (copy_to_user
		    ((void __user *)arg, (void *)FP_DEV_VERSION,
		     sizeof(char) * 7)) {
	...
	case SIFP_IOC_SET_KMAP:
		...
		if (copy_from_user
		    (&fp_dev->keymap_cust.k, (void __user *)arg,
		     sizeof(struct fp_dev_kmap_t))) {
			LOG_MSG_DEBUG(ERR_LOG,
				      "[IOC_SET_KMAP] copy_from fail\n");
			retval = -EFAULT;
			break;
		}
	...
	case SIFP_IOC_PKG_SIZE:
		if (arg) {
			LOG_MSG_DEBUG(DBG_LOG, "[IOC_PKG_SIZE] %d\n",
				      (u8) (PKG_SIZE));
			retval =
			    __put_user((u8) (PKG_SIZE), (__u8 __user *) arg);
		}
		break;
	...
	case SIFP_IOC_DBG_LEVEL:
		if (arg) {
			...
			if (copy_from_user
			    (&level, (void __user *)arg, sizeof(char))
	...
	case SIFP_IOC_WAKELOCK:
		if (arg) {
			...
			if (copy_from_user
			    (&lock, (void __user *)arg, sizeof(char))) {
				LOG_MSG_DEBUG(ERR_LOG,
					      "[SIFP_IOC_WAKELOCK] copy_from fail\n");
				retval = -EFAULT;
				break;
			}
	...
	case SIFP_IOC_PWDN:
		if (arg) {
			...
			if (copy_from_user
			    (&flag_avdd, (void __user *)arg, sizeof(char))) {
				LOG_MSG_DEBUG(ERR_LOG,
					      "[SIFP_IOC_PWDN] copy_from fail\n");
				retval = -EFAULT;
				break;
			}
	...
	case SIFP_IOC_PROC_NODE:
		...
		if (arg) {
			if (copy_from_user
			    (vendor_name, (void __user *)arg,
			     PROC_VND_ID_LEN)) {
				LOG_MSG_DEBUG(ERR_LOG,
					      "[SIFP_IOC_PROC_NODE] copy_from fail\n");
				retval = -EFAULT;
				break;
			}
	...
	case SIFP_IOC_SET_FEATURE:
		if (arg) {
			...
			if (copy_from_user
			    (&feature, (void __user *)arg, sizeof(char))) {
				LOG_MSG_DEBUG(ERR_LOG,
					      "[SIFP_IOC_SET_FEATURE] copy_from fail\n");
				retval = -EFAULT;
				break;
			}
...
int silfp_touch_event_handler(struct fp_dev_touch_info *tp_info)
{
	...
	LOG_MSG_DEBUG(INFO_LOG, "tp_info %d, %d, %d, %d \n",
		      tp_info->touch_state, tp_info->area_rate, tp_info->x,
		      tp_info->y);
...
static int silfp_release(struct inode *inode, struct file *filp)
{
	...
	mutex_lock(&device_list_lock);
	fp_dev = filp->private_data;
	/*HS07 code for SR-AL7761A-01-522 by yuli at 20250319 start */
	silfp_resource_deinit(fp_dev);
	/*HS07 code for SR-AL7761A-01-522 by yuli at 20250319 end */
...
static int silfp_probe(struct spi_device *spi)
{
	...
	wake_lock_init(&fp_dev->wakelock_hal, WAKE_LOCK_SUSPEND,
		       "silfp_wakelock_hal");
	...
	if (FP_DEV_MAJOR > 0) {
		...
		status =
		    alloc_chrdev_region(&fp_dev->devt, fp_dev->ref++, 1,
					FP_DEV_NAME);
	}
	if (status < 0) {
	...
	fp_dev->dev =
	    device_create(silfp_class, &fp_dev->spi->dev, fp_dev->devt, fp_dev,
			  FP_DEV_NAME);
...
MODULE_DESCRIPTION
    ("Gigadevice/Silead Fingerprint driver for GSL6xxx/GSL7xxx/GSL8xxx series.");
MODULE_LICENSE("GPL");
