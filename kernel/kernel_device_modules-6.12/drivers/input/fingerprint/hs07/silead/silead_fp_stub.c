...
static long sil_stub_ioctl(struct file *filp, unsigned int cmd,
			   unsigned long arg)
{
	unsigned long flags;
...
static int __init silead_stub_init(void)
{
	...
	if (!sil_stub_devp) {
		sil_stub_devp =
		    kzalloc(sizeof(struct sil_stub_dev), GFP_KERNEL);
		if (!sil_stub_devp) {
			return -ENOMEM;
		}
	}
	...
	if (SIL_STUB_MAJOR > 0) {
		...
		ret =
		    register_chrdev_region(sil_stub_devp->devt, 1,
						FP_STUB_DEV_NAME);
	} else {
		ret =
		    alloc_chrdev_region(&sil_stub_devp->devt, 0, 1,
					FP_STUB_DEV_NAME);
	}
	...
	device_create(sil_stub_class, NULL, sil_stub_devp->devt, NULL,
			FP_STUB_DEV_NAME);
