...
static int vreg_setup(struct anc_data *p_data, const char *name, bool enable)
{
	...
	if (enable) {
		...
		if (regulator_count_voltages(p_data->vreg) > 0) {
			ret_val =
			    regulator_set_voltage(p_data->vreg, vreg_conf.vmin,
						  vreg_conf.vmax);
			if (ret_val) {
				ANC_LOGE("Unable to set voltage on %s, %d",
					 name, ret_val);
			}
		}
		...
		if (ret_val < 0) {
			ANC_LOGE("Unable to set current on %s, %d", name,
				 ret_val);
		}
...
static int select_pin_ctl(struct anc_data *p_data, const char *name)
{
	...
	for (i = 0; i < ARRAY_SIZE(p_data->pinctrl_state); i++) {
		...
		if (!strncmp(n, name, strlen(n))) {
			ret_val =
			    pinctrl_select_state(p_data->fingerprint_pinctrl,
						 p_data->pinctrl_state[i]);
			if (ret_val) {
				ANC_LOGE("cannot select %s, ret_val: %d", name,
					 ret_val);
			} else {
...
static int set_reset_level(struct anc_data *p_data, int level)
{
	...
	if (ret_val != 0) {
		ANC_LOGE("set reset level:%d failed, ret_val: %d", level,
			 ret_val);
	}
...
static void anc_power_onoff(struct anc_data *p_data, bool power_onoff)
{
	...
	if (p_data->vdd_use_gpio) {
		if (!power_onoff) {
			...
			if (ret_val) {
				ANC_LOGD("set fp_gpio_mode, ret_val:%d",
					 ret_val);
			} else {
				mdelay(1);
			}
	...
	if (p_data->vdd_use_pmic) {
		int ret_val =
		    vreg_setup(p_data, ANC_VREG_VDD_NAME, power_onoff);
		if (ret_val != 0) {
			ANC_LOGE("use pmic to contrl power failed, ret_val: %d",
				 ret_val);
		}
	}
}
...
static void anc_wake_unlock(struct anc_data *p_data)
{
	...
	__pm_wakeup_event(p_data->fp_wakelock,
			  msecs_to_jiffies(ANC_WAKELOCK_HOLD_TIME));
#else
	wake_unlock(&p_data->fp_wakelock);
	wake_lock_timeout(&p_data->fp_wakelock,
			  msecs_to_jiffies(ANC_WAKELOCK_HOLD_TIME));
#endif
}
 ...
 */
static ssize_t resource_control_set(struct device *dev,
...
static int anc_request_named_gpio(struct anc_data *p_data, const char *label,
				  int *gpio)
{
	int ret_val = 0;

	if ((p_data == NULL) || (label == NULL) || (gpio == NULL)) {
		ANC_LOGE("parameter is NULL, p_data:%p, label:%p, gpio:%p",
			 p_data, label, gpio);
		return -EINVAL;
	}
...
static int anc_irq_init(struct anc_data *p_data)
{
	...
	if (p_data->irq_init == 0) {
		ret_val =
		    devm_request_threaded_irq(p_data->dev, p_data->irq, NULL,
					      anc_irq_handler, irqf,
					      dev_name(p_data->dev), p_data);
		if (ret_val != 0) {
			ANC_LOGE("Could not request irq %d, ret_val: %d",
				 p_data->irq, ret_val);
			return ret_val;
		}
		p_data->irq_init = 1;
...
static int anc_report_key_event(struct anc_data *p_data, const void *arg)
{
	...
	ret_val =
	    anc_copy_from_user(&(p_data->key_event), arg,
			       sizeof(ANC_KEY_EVENT));
	if (ret_val != 0) {
		ANC_LOGE
		    ("Failed to copy input key event from user to kernel, ret_val: %d",
		     ret_val);
		return -EFAULT;
	}

	ANC_LOGD("key = %d, value = %d", p_data->key_event.key,
		 p_data->key_event.value);
...
static int use_pinctrl_init(struct anc_data *p_data)
{
	...
	for (i = 0; i < ARRAY_SIZE(p_data->pinctrl_state); i++) {
		struct pinctrl_state *state =
		    pinctrl_lookup_state(p_data->fingerprint_pinctrl,
					 pctl_names[i]);
		if (IS_ERR(state)) {
...
static int use_gpio_init(struct anc_data *p_data)
{
	...
	ret_val =
	    anc_request_named_gpio(p_data, "anc,gpio_rst", &p_data->rst_gpio);
	if (ret_val) {
	...
	if (ret_val) {
		ANC_LOGD("reset gpio direction output failed, ret_val:%d",
			 ret_val);
		return ret_val;
	}

	ret_val =
	    anc_request_named_gpio(p_data, "anc,gpio_irq", &p_data->irq_gpio);
	if (ret_val) {
	...
	if (ret_val) {
		ANC_LOGD("irq gpio direction input failed, ret_val:%d",
			 ret_val);
		return ret_val;
	}
...
static int anc_gpio_init(struct anc_data *p_data)
{
	...
	if (p_data->vdd_use_gpio) {
		ret_val =
		    anc_request_named_gpio(p_data, "anc,gpio_pwr",
					   &p_data->pwr_gpio);
		if (ret_val) {
			ANC_LOGE("request power gpio failed, ret_val:%d",
				 ret_val);
			goto ANC_INIT_FAIL;
		}
		...
		if (ret_val) {
			ANC_LOGE
			    ("power gpio direction output failed, ret_val:%d",
			     ret_val);
			goto ANC_INIT_FAIL;
		}
	}
	...
	if (p_data->use_gpio_init == false) {
		...
		if (ret_val) {
			ANC_LOGE("use pinctrl init failed, ret_val:%d",
				 ret_val);
			goto ANC_INIT_FAIL;
		}
	}
...
static int anc_spi_clk_enable(struct anc_data *data, uint32_t flag)
{
	...
	if (ANC_LOW_SPEED == flag) {
		...
		ANC_LOGE("fail to enable clock, flag(%d) is not in range[0-2]",
			 flag);
	}
...
static int anc_pm_qos_opp_init(void)
{
	...
	exynos_pm_qos_add_request(&pm_qos_req_cpu[0], PM_QOS_CLUSTER0_FREQ_MIN,
				  0);
	exynos_pm_qos_add_request(&pm_qos_req_cpu[1], PM_QOS_CLUSTER1_FREQ_MIN,
				  0);
...
static int set_product_id(struct anc_data *p_data, const void *arg)
{
	...
	ret_val =
	    anc_copy_from_user(&(p_data->product_info), arg,
			       sizeof(ANC_SENSOR_PRODUCT_INFO));
	if (ret_val != 0) {
		ANC_LOGE("Failed to copy product info from user to kernel");
		return ret_val;
	}
	ANC_LOGD("set product id: %s", p_data->product_info.product_id);
	custom_send_command(CUSTOM_COMMAND_SET_PRODUCT_ID,
			    p_data->product_info.product_id);
...
static int anc_create_device(struct anc_data *p_data)
{
	...
	if (ret_val < 0) {
		ANC_LOGE("Failed to alloc char device region, ret_val: %d",
			 ret_val);
	...
	device_ptr =
	    device_create(p_data->dev_class, NULL, p_data->dev_num, p_data,
			  ANC_DEVICE_NAME);
...
static int anc_parse_dts(struct anc_data *p_data)
{
	...
	p_data->vdd_use_gpio =
	    of_property_read_bool(p_data->dev->of_node, "anc,vdd_use_gpio");
	p_data->vdd_use_pmic =
	    of_property_read_bool(p_data->dev->of_node, "anc,vdd_use_pmic");
	p_data->use_gpio_init =
	    of_property_read_bool(p_data->dev->of_node, "anc,use_gpio_init");
	ANC_LOGD("vdd_use_gpio = %d, vdd_use_pmic = %d, use_gpio_init = %d",
		 p_data->vdd_use_gpio, p_data->vdd_use_pmic,
		 p_data->use_gpio_init);

	if (p_data->vdd_use_pmic) {
		vreg_conf.name = ANC_VREG_VDD_NAME;
		ret_val =
		    of_property_read_u32_index(p_data->dev->of_node,
					       ANC_VDD_CONFIG_NAME, 0,
					       &vreg_conf.vmin);
		if (ret_val != 0) {
		...
		ANC_LOGD
		    ("vreg config: name = %s, vmin = %d, vmax = %d, ua_load = %d",
		     vreg_conf.name, vreg_conf.vmin, vreg_conf.vmax,
		     vreg_conf.ua_load);
	}
...
#else
	wake_lock_init(&p_data->fp_wakelock, WAKE_LOCK_SUSPEND,
			   "anc_fp_wakelock");
#endif
...
/*HS07 code for HS07-955 by hehaoran at 20250418 start*/
#if IS_ENABLED(CONFIG_DRM_MEDIATEK_V2)
	mtk_disp_notifier_unregister(&p_data->notif);
#else//CONFIG_DRM_MEDIATEK_V2
	fb_unregister_client(&p_data->notif);
#endif//CONFIG_DRM_MEDIATEK_V2
#ifdef ANC_SUPPORT_NAVIGATION_EVENT
	input_unregister_device(p_data->input);
#endif//ANC_SUPPORT_NAVIGATION_EVENT
/*HS07 code for HS07-955 by hehaoran at 20250418 end*/
	anc_destroy_device(p_data);
