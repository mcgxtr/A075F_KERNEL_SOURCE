...
typedef struct phase_s
{
    ...
    unsigned long prox4_mask;
    unsigned long prox3_mask;
    unsigned long prox2_mask;
    unsigned long prox1_mask;
    char phase_status;
    int keycode_far;
    int keycode_close;
    bool enabled;
...
#else
/*Tab A11 code for AX7800A-650 by lichang at 20240512 start*/
static phase_t smtc_phase_table[] =
{
    {
...
/*Tab A11 code for AX7800A-650 by lichang at 20240512 end*/
#endif
...
static void smtc_anfr_irq(Self self)
{
    ...
    if (((prox_state >> 26) & 1) != gs_last_state) {
        g_irq_count++;
        gs_last_state = (prox_state >> 26) & 1;
...
static int smtc_parse_dts(Self self)
{
    ...
    /*Tab A11 code for AX7800A-612|AX7800A-650 by lichang at 20240512 start*/
    #ifdef CONFIG_CUSTOM_PROJECT_HST11_WIFI
        self->main_phases = 0x08;
        self->ref_phases = 0x10;
        SMTC_LOG_DBG("main-phases=0x%X, ref-phases=0x%X", self->main_phases, self->ref_phases);
    #endif
    /*Tab A11 code for AX7800A-612|AX7800A-650 by lichang at 20240512 end*/
...
static int smtc_init_registers(Self self)
{
    ...
    for (i=0; i < self->num_regs; i++)
    {
        ...
        if (reg_addr == 0x4004 && (reg_val & 1<<4)==0){
            SMTC_LOG_WRN("0x4004=0x%X, usually compensation IRQ should be enabled.",reg_addr);
        }
        ...
        if(phen & 1<<ph)
        {
            ...
            for(idx=1; idx<_ESD_FAIL_CHECK_TIMES; idx++)
            {
                ...
                if (tmp_u32 == SATURATED_USEFUL){
                    ...
                    SMTC_LOG_DBG("ph=%d useful=0x%X is saturated, skip checking this phase",
                        ph, tmp_u32);
                    break;
                }
...
static void smtc_log_raw_data(Self self)
{
    ...
    ref_ph_b = self->ref_phase_b;
    ref_ph_c = self->ref_phase_c;
    SMTC_LOG_DBG("ref_ph_a=%d,ref_ph_b=%d,ref_ph_c=%d", ref_ph_a,ref_ph_b,ref_ph_c);
...
/*Tab A11 code for AX7800A-650 by lichang at 20240512 start*/
/*Tab A11 code for AX7800A-612 by lichang at 20240424 start*/
/*Tab A11 code for P250910-01527|P250910-01309 by wangtegfei at 20250915 start*/
static ssize_t smtc_set_dumpinfo(const char *buf, size_t count)
{
    ...
    SMTC_LOG_INF("dumpinfo value %d, g_channel %d", value, g_channel);

    return count;
}
/*Tab A11 code for AX7800A-612 by lichang at 20240424 end*/
/*Tab A11 code for P250910-01527|P250910-01309 by wangtegfei at 20250915 end*/
...
static ssize_t smtc_get_dumpinfo(char *buf)
{
    ...
    if (g_self != NULL) {
        ...
        g_dumpinfo.total_cap = ((((g_dumpinfo.total_cap & 0x00003F80) >> 7) * 3100) +
        ...
        if (g_channel == CH1 || g_channel == CH2 || g_channel == CH3) {
            g_dumpinfo.ref_cap = ((((g_dumpinfo.ref_cap & 0x00003F80) >> 7) * 3100) +
...
static int smtc_pcbainfo_judge(void)
{
    ...
    if (board_id == 0x00 || board_id == 0x06) {
        ...
        SMTC_LOG_INF("is eur wifi");
        return WIFI_ONLY;
    }else {
        SMTC_LOG_INF("is not eu");
        return NONE;
    }
}
/*Tab A11 code for AX7800A-765 by lichang at 20240516 end*/
...
/*Tab A11 code for AX7800A-3476 by zhawei at 20240707 end*/
    ...
    self->channel_status = 0;
    self->irq_wake_status = false;
    g_usernode.sar_name_drv = "sx937x";
