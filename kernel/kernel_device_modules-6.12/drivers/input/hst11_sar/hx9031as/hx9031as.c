...
#endif
/*Tab A11 code for AX7800A-326|AX7800A-327 by lichang at 20240415 end*/

/*Tab A11 code for AX7800A-765 by lichang at 20240516 start*/
enum area_info {
    NONE = -1,
    EUR,        //000/110
    WIFI_ONLY,  //100
};
/*Tab A11 code for AX7800A-765 by lichang at 20240516 end*/

/*Tab A11 code for AX7800A-612 by lichang at 20240424 start*/
//After sales tools issue SAR dumpinfo information.
typedef enum{
    GRIP_MAIN = 1,
    GRIP_SUB,
    GRIP_WIFI,
...
/*Tab A11 code for AX7800A-612 by lichang at 20240424 end*/

/*Tab A11 code for AX7800A-3476 by zhawei at 20240707 start*/
#ifndef ANFR_IMPORT_ALL
extern int g_board_id_status;
static int g_sarpcbinfo = -1;
#endif    // ANFR_IMPORT_ALL
...
/*Tab A11 code for AX7800A-1057 by zhawei at 20250527 start*/
static uint8_t dts_reg_dump = 0;
static struct hx9031as_addr_val_pair *dts_reg_init = NULL;
/*Tab A11 code for AX7800A-1057 by zhawei at 20250527 end*/
...
/*Tab A11 code for AX7800A-1057 by zhawei at 20250527 start*/
#ifdef DTS_REG_INIT
static void hx9031as_dts_reg_init(void)
{
    int ii = 0;
    int ret = -1;

    while(ii < hx9031as_pdata.num_regs) {
        ret = hx9031as_write(dts_reg_init[ii].addr, &dts_reg_init[ii].val, 1);
        if(0 != ret) {
            PRINT_ERR("hx9031as_dts_reg_init write failed\n");
        }
    ii++;
    }
}
...
static void tyhx_anfr_irq(void)
{
    ...
    touch_state = (hx9031as_pdata.prox_state_reg >> 3) & 0x1;
...
/*hs07 code for HS07-26 by huofudong at 20250225 start*/
...
/*hs07 code for HS07-26 by huofudong at 20250225 end*/
...
static ssize_t hx9031_set_dumpinfo(const char *buf, size_t count)
{
    ...
    if (value == GRIP_MAIN) {
        g_channel1 = CH1;
    } else if (value == GRIP_SUB) {
        g_channel1 = CH4;
    } else if (value == GRIP_WIFI) {
        g_channel1 = CH3;
    } else {
        g_channel1 = CH1;
    }
    PRINT_DBG("dumpinfo value %d, g_channel %d", value, g_channel1);

    return count;
}
/*Tab A11 code for AX7800A-612 by lichang at 20240424 end*/

/*hs07 code for HS07-3484 by lichang at 20250527 start*/
static ssize_t hx9031_get_dumpinfo(char *buf)
{
    ...
    total_cap = ((data_offset_dac[g_channel1] / 4) * 234) / 1000;

    if(g_channel1 == CH1) {
        ref_cap = ((data_offset_dac[CH0] / 4) * 234) / 1000;
    } else if (g_channel1 == CH3) {
        ref_cap = ((data_offset_dac[CH2] / 4) * 234) / 1000;
    } else {
        ref_cap = 0;
    }

    return snprintf(buf, PAGE_SIZE, "%u,%d,%d,%d,%d,%d,%d\n",
            hx9031as_pdata.fw_num, state, total_cap, ref_cap,
            data_lp[g_channel1], data_offset_dac[g_channel1],data_diff[g_channel1]);
}
/*hs07 code for HS07-3484 by lichang at 20250527 end*/
...
static int hx9031as_parse_dt(struct device *dev)
{
    ...
    /***********************************************************************/
    //retrieve register parameters from dts

    ret = of_property_read_u32(dt_node, "tyhx,reg-num", &hx9031as_pdata.num_regs);
    if (ret < 0) {
        PRINT_ERR("failed to get tyhx,reg-num\n");
        return -1;
    }
    PRINT_INF("hx9031as_pdata.num_regs = %d\n", hx9031as_pdata.num_regs);

    if (hx9031as_pdata.num_regs > 0) {
        dts_reg_init = devm_kzalloc(dev,sizeof(struct hx9031as_addr_val_pair) * hx9031as_pdata.num_regs, GFP_KERNEL);
        if (dts_reg_init == NULL) {
            return -1;
        }
    }
    ret = of_property_read_u32_array(dt_node,"tyhx,reg-init",(u32*)&(dts_up[0]),DTS_REG_NUM);

    for ( i = 0; i < hx9031as_pdata.num_regs; i++) {
        dts_reg_init[i].addr = dts_up[i*num];
        dts_reg_init[i].val = dts_up[(num*i)+1];
        PRINT_INF("0x%02X=0x%02X\n", dts_reg_init[i].addr, dts_reg_init[i].val);
    }

    if(ret < 0) {
        PRINT_ERR("failed to get tyhx,reg-init\n");
        return -1;
    }
    return 0;
}
...
static ssize_t hx9031as_dump_show(const struct class *class,const struct class_attribute *attr, char *buf)
{
    ...
    if(dts_reg_dump == 1) {
        for(ii = 0; ii < hx9031as_pdata.num_regs; ii++) {
            ret = hx9031as_read(dts_reg_init[ii].addr, rx_buf, 1);
            if(0 != ret) {
                PRINT_ERR("hx9031as_read failed\n");
            }
            PRINT_INF("0x%02X=0x%02X\n", dts_reg_init[ii].addr, rx_buf[0]);
...
static int hx9031as_input_init_key(struct i2c_client *client)
{
    ...
    hx9031as_pdata.chs_info[4].keycode_close = KEY_SAR2_CLOSE;
    hx9031as_pdata.chs_info[4].keycode_far = KEY_SAR2_FAR;
    hx9031as_pdata.chs_info[3].keycode_close = KEY_SAR3_CLOSE;
    hx9031as_pdata.chs_info[3].keycode_far = KEY_SAR3_FAR;
    /*Tab A11 code for AX7800A-618 by lichang at 20240427 start*/
    #ifdef CONFIG_CUSTOM_PROJECT_HST11_WIFI
        hx9031as_pdata.chs_info[1].used = false;
        hx9031as_pdata.chs_info[4].used = false;
    #else
        snprintf(hx9031as_pdata.chs_info[1].name,sizeof(hx9031as_pdata.chs_info[1].name),"grip_sensor");
        snprintf(hx9031as_pdata.chs_info[4].name,sizeof(hx9031as_pdata.chs_info[4].name),"grip_sensor_sub");
    #endif
    /*Tab A11 code for AX7800A-618 by lichang at 20240427 end*/
...
static int hx9031as_input_init_abs(struct i2c_client *client)
{
    ...
    hx9031as_pdata.chs_info[4].phase_status = 0x02;
    hx9031as_pdata.chs_info[3].phase_status = 0x04;
    hx9031as_pdata.chs_info[0].used = false;
    hx9031as_pdata.chs_info[2].used = false;
...
#ifndef ANFR_IMPORT_ALL
/*Tab A11 code for AX7800A-765 by lichang at 20240516 start*/
static int tyhx_pcbainfo_judge(void)
{
    int board_id = 0;

    board_id = g_board_id_status & 0x07;
    PRINT_ERR("[%d, %d]", g_board_id_status, board_id);

    if (board_id == 0x00 || board_id == 0x06) {
        PRINT_ERR("is eur");
        return EUR;
    } else if (board_id == 0x04) {
        PRINT_ERR("is eur wifi");
        return WIFI_ONLY;
    }else {
        PRINT_ERR("is not eur");
        return NONE;
    }
}
/*Tab A11 code for AX7800A-765 by lichang at 20240516 end*/
...
/*Tab A11 code for AX7800A-3476 by zhawei at 20240707 start*/
#ifndef ANFR_IMPORT_ALL
    g_sarpcbinfo = tyhx_pcbainfo_judge();
    if (g_sarpcbinfo == NONE) {
        #ifdef HX9031AS_USER_NODE
        g_anfr_flag = false;
        #endif //HX9031AS_USER_NODE
    }
#endif    // ANFR_IMPORT_ALL
/*Tab A11 code for AX7800A-3476 by zhawei at 20240707 end*/
...
/*Tab A11 code for AX7800A-1056 by zhawei at 20250527 start*/
static int hx9031as_suspend(struct device *dev)
{
    int ret = -1;
    uint8_t rx_buf[3] = {0x22,0x22,0x02};
    uint8_t rx_buf1[3] = {0x66,0x66,0x16};

    ENTER;
    hx9031as_irq_from_suspend_flag = 1;

    ret = hx9031as_write(RW_C0_CH10_SCAN_FACTOR, &rx_buf[0], 1);
    ret |= hx9031as_write(RW_C1_CH32_SCAN_FACTOR, &rx_buf[1], 1);
    ret |= hx9031as_write(RW_C7_CH4_FACTOR_CTRL, &rx_buf[2], 1);
    ret |= hx9031as_write(RW_2C_UP_ALP_1_0_CFG, &rx_buf1[0], 1);
    ret |= hx9031as_write(RW_2D_UP_ALP_3_2_CFG, &rx_buf1[1], 1);
    ret |= hx9031as_write(RW_2E_DN_UP_ALP_0_4_CFG, &rx_buf1[2], 1);
    if(0 != ret) {
        PRINT_ERR("hx9031as_write failed\n");
    }
    PRINT_DBG("switch to low-power mode\n");
...
static int hx9031as_resume(struct device *dev)
{
    ...
    uint8_t rx_buf1[3] = {0x88,0x88,0x18};
    ...
    PRINT_DBG("switch to normal mode\n");

    return 0;
}
/*Tab A11 code for AX7800A-1056 by zhawei at 20250527 end*/
