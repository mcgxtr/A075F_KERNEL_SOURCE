...
struct upm6720 {
    ...
    struct delayed_work check_vac_work;
...
/*HS07 code for HS07-1245 by xiongxiaoliang at 20250423 start*/
static void upm6720_check_vac_work(struct work_struct *work)
{
    struct upm6720 *upm = container_of(
            work, struct upm6720, check_vac_work.work);

    u8 val = 0;
    u8 vac = 0;
    u8 val1 = 0;

    upm_info("disable OTG,vac_online,check vac\n");
    upm6720_read_byte(upm, UPM6720_REG_15, &val);
    vac = !!(val & UPM6720_VAC1_PRESENT_STAT_MASK);
    if (vac) {
        schedule_delayed_work(&upm->check_vac_work, msecs_to_jiffies(500));
    } else {
        val1 = UPM6720_DIS_ACDRV_BOTH_DISABLE;
        upm6720_update_bits(upm, UPM6720_REG_0F, UPM6720_DIS_ACDRV_BOTH_MASK, val1 <<= UPM6720_DIS_ACDRV_BOTH_SHIFT);
        upm_info("disable OTG,vac_offline,change to atuo mode\n");
    }
}
/*HS07 code for HS07-1245 by xiongxiaoliang at 20250423 end*/
...
static void upm6720_set_otg_txmode(struct upm6720 *upm, int enable)
{
    int ret = 0;
    /*HS07 code for HS07-1245 by xiongxiaoliang at 20250423 start*/
    u8 val,val1,val2,val3,vac;

    if (enable) {
        val1 = UPM6720_DIS_ACDRV_BOTH_ENABLE;
        val2 = UPM6720_EN_BYPASS_ENABLE;
        upm6720_enable_charge(upm, 0);
        ret |= upm6720_update_bits(upm, UPM6720_REG_0F,UPM6720_DIS_ACDRV_BOTH_MASK, val1 <<= UPM6720_DIS_ACDRV_BOTH_SHIFT);
        ret |= upm6720_update_bits(upm, UPM6720_REG_0F,UPM6720_EN_BYPASS_MASK, val2 <<= UPM6720_EN_BYPASS_SHIFT);
    } else {
        ret = upm6720_read_byte(upm, UPM6720_REG_15, &val);
        vac = !!(val & UPM6720_VAC1_PRESENT_STAT_MASK);
        if (vac) {
            upm_info("vac is online wait ~\n");
            schedule_delayed_work(&upm->check_vac_work, msecs_to_jiffies(200));
        } else {
            val1 = UPM6720_EN_BYPASS_DISABLE;
            val2 = UPM6720_DIS_ACDRV_BOTH_DISABLE;
            val3 = UPM6720_AC1_PD_ENABLE;
            ret |= upm6720_update_bits(upm, UPM6720_REG_0F,UPM6720_EN_BYPASS_MASK, val1 <<= UPM6720_EN_BYPASS_SHIFT);
            ret |= upm6720_update_bits(upm, UPM6720_REG_0F,UPM6720_DIS_ACDRV_BOTH_MASK, val2 <<= UPM6720_DIS_ACDRV_BOTH_SHIFT);
            ret |= upm6720_update_bits(upm, UPM6720_REG_0E,UPM6720_AC1_PD_EN_MASK, val3 <<= UPM6720_AC1_PD_EN_SHIFT);
        }
    }
...
/*HS07 code for HS07-320 by huofudong at 20250402 start*/
int upm6720_usbpd_pm_set_shipmode_adc(int *value)
{
    int enable = *value;

    if (g_upm != NULL) {
        upm6720_enable_adc(g_upm, enable);
        return true;
    } else {
        return false;
    }
}
/*HS07 code for HS07-320 by huofudong at 20250402 end*/
...
void upm6720_usbpd_setvacovp(int *value)
{
    int enable = *value;

    if (g_upm != NULL) {
        if (enable == true){
            upm6720_set_vac1ovp_th(g_upm, 6500);
        } else {
            upm6720_set_vac1ovp_th(g_upm, 12000);
        }
    }
}
...
static int upm6720_irq_register(struct upm6720 *upm)
{
    ...
    ret = gpio_direction_input(upm->irq_gpio);
    if (ret) {
...
static int upm6720_charger_probe(struct i2c_client *client)
{
    ...
    /*HS07 code for HS07-1245 by xiongxiaoliang at 20250423 start*/
    INIT_DELAYED_WORK(&upm->check_vac_work, upm6720_check_vac_work);
    /*HS07 code for HS07-1245 by xiongxiaoliang at 20250423 end*/
    //schedule_delayed_work(&upm->monitor_work, msecs_to_jiffies(3*1000));
    ...
    /*HS07 code for HS07-3571 by yexuedong at 20250530 start*/
    g_gxy_cp_ops.set_usbpd_setvacovp = upm6720_usbpd_setvacovp;
    /*HS07 code for HS07-3571 by yexuedong at 20250530 end*/
    g_gxy_cp_ops.get_otg_txmode = upm6720_usbpd_get_otg_txmode;
    upm->otg_tx_mode = false;
    /*A06_V code for SR-AL7160V-01-92 by xiongxiaoliang at 20240904 end*/
    /*HS07 code for HS07-320 by huofudong at 20250402 start*/
    g_gxy_cp_ops.set_shipmode_adc = upm6720_usbpd_pm_set_shipmode_adc;
    /*HS07 code for HS07-320 by huofudong at 20250402 end*/
    /*HS07 code for HS07-215 by huofudong at 20250326 start*/
    gxy_bat_set_cpinfo(GXY_BAT_CP_INFO_UPM6726);
    /*HS07 code for HS07-215 by huofudong at 20250326 end*/
