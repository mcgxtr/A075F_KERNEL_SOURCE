...
/* A07 LTE_BOS code for AL7761C-1 by wangmingye at 20250808 start */
#define PSY_NAME_SW_CHARGER     "primary_chg"
/* A07 LTE_BOS code for AL7761C-1 by wangmingye at 20250808 end */
...
struct pdpm_config {
    ...
    int bus_curr_lp_lmt;
    /*HS07 code for HS07-465 by xiongxiaoliang at 20250408 start*/
    int jeita_cv_ibat_limit;
    int jeita_cv_vbat_limit;
    int jeita_cv_status;
    /*HS07 code for HS07-465 by xiongxiaoliang at 20250408 end*/
...
/*HS07 code for HS07-2413 by xiongxiaoliang at 20250428 start*/
#define MAX_REQUEST_POWER         26000000
/*HS07 code for HS07-2413 by xiongxiaoliang at 20250428 end*/
...
/*HS07 code for HS07-389 by xiongxiaoliang at 20250407 start*/
#define BAT_CV_OFFSET 35 //cv-50mv
/*HS07 code for HS07-389 by xiongxiaoliang at 20250407 end*/
...
#define JEITA_TEMP_T2_TO_T3_CP_CV3 4450
#define JEITA_TEMP_T3_TO_T4_CP_CV1 4250
/*HS07 code for HS07-2413 by xiongxiaoliang at 20250428 start*/
#define JEITA_CV_OFFSET 6
#define JEITA_CC_OFFSET 100
/*HS07 code for HS07-2413 by xiongxiaoliang at 20250428 end*/
/*HS07 code for HS07-465 by xiongxiaoliang at 20250408 end*/
...
void usbpd_pm_set_otg_txmode(int enable)
{
    ...
    if (g_gxy_cp_ops.set_otg_txmode) {
        ...
        if (ret) {
            /*HS07 code for HS07-1245 by xiongxiaoliang at 20250423 start*/
            pr_err("set otg txmode ok [%d]\n", enable);
            /*HS07 code for HS07-1245 by xiongxiaoliang at 20250423 end*/
            return;
        }
    }
...
void usbpd_setvacovp(int enable)
{
    if (g_gxy_cp_ops.set_usbpd_setvacovp) {
        g_gxy_cp_ops.set_usbpd_setvacovp(&enable);
    }
    pr_err("set upm usbpd_setvacovp %d", enable);
}
EXPORT_SYMBOL(usbpd_setvacovp);
...
void usbpd_pm_set_shipmode_adc(int enable)
{
    int ret = 0;
    if (g_gxy_cp_ops.set_shipmode_adc) {
        ret = g_gxy_cp_ops.set_shipmode_adc(&enable);
        if (ret) {
            pr_err("set shipmode adc ok [%d]\n", ret);
            return;
        }
    }
    pr_err("set shipmode adc fail [%d]\n", ret);
}
EXPORT_SYMBOL(usbpd_pm_set_shipmode_adc);
...
static int usbpd_select_pdo(struct usbpd_pm *pdpm, u32 mV, u32 mA)
{
    ...
    /*HS07 code for HS07-200 by xiongxiaoliang at 20250325 start*/
    if (pdpm->is_pps_en_unlock == false) {
        pdpm->pd_active = false;
        pr_err("pd trigger hardreset, exit cp\n");
        return -EINVAL;
    }
    /*HS07 code for HS07-200 by xiongxiaoliang at 20250325 end*/
...
static int usbpd_pm_fc2_charge_algo(struct usbpd_pm *pdpm)
{
    ...
    */
    ...
    /*T2-T3: CV1 - CV2*/
    /*HS07 code for HS07-2413 by xiongxiaoliang at 20250428 start*/
    if (g_cp_charging_lmt.cp_jeita_cc / 1000 == JEITA_TEMP_T2_TO_T3_CP_CC1) {
        if (vbat_sw_now > JEITA_TEMP_T2_TO_T3_CP_CV2 - JEITA_CV_OFFSET) {
            pr_err("cp enter cv2\n");
            pm_config.jeita_cv_status = JEITA_CV_2;

            if (pdpm->sw.ibat_curr < JEITA_TEMP_T2_TO_T3_CP_CC3) {
                pr_err("cp exit cv2\n");
                pm_config.jeita_cv_status = JEITA_CV_2_CC;
            }
        } else if (vbat_sw_now > JEITA_TEMP_T2_TO_T3_CP_CV1 - JEITA_CV_OFFSET) {
            pr_err("cp enter cv1\n");
            pm_config.jeita_cv_status = JEITA_CV_1;

            if (pdpm->sw.ibat_curr < JEITA_TEMP_T2_TO_T3_CP_CC2) {
                pr_err("cp exit cv1\n");
                pm_config.jeita_cv_status = JEITA_CV_1_CC;
            }
        } else {
            pm_config.jeita_cv_status = JEITA_CV_INIT;
        }
    } else {
        pm_config.jeita_cv_status = JEITA_CV_INIT;
    }

    switch (pm_config.jeita_cv_status) {
        case JEITA_CV_INIT:
            pm_config.jeita_cv_ibat_limit = JEITA_TEMP_T2_TO_T3_CP_CC1;
            pm_config.jeita_cv_vbat_limit = JEITA_TEMP_T2_TO_T3_CP_CV3;
            break;
        case JEITA_CV_1:
            pm_config.jeita_cv_vbat_limit = JEITA_TEMP_T2_TO_T3_CP_CV1 + BAT_CV_OFFSET;
            break;
        case JEITA_CV_1_CC:
        case JEITA_CV_2:
            pm_config.jeita_cv_vbat_limit = JEITA_TEMP_T2_TO_T3_CP_CV2 + BAT_CV_OFFSET;
            pm_config.jeita_cv_ibat_limit = JEITA_TEMP_T2_TO_T3_CP_CC2 - JEITA_CC_OFFSET;
            break;
        case JEITA_CV_2_CC:
            pm_config.jeita_cv_vbat_limit = JEITA_TEMP_T2_TO_T3_CP_CV3;
            pm_config.jeita_cv_ibat_limit = JEITA_TEMP_T2_TO_T3_CP_CC3 - JEITA_CC_OFFSET;
            break;
        default:
            break;
    }
    /*HS07 code for HS07-2413 by xiongxiaoliang at 20250428 end*/
    pr_err("jeita_cv_ibat_limit(%d), jeita_cv_vbat_limit(%d), cv1_cv2_sts(%d)\n",
        pm_config.jeita_cv_ibat_limit, pm_config.jeita_cv_vbat_limit, pm_config.jeita_cv_status);
    /*HS07 code for HS07-389 by xiongxiaoliang at 20250407 end*/
    pm_config.bat_curr_lp_lmt = min(pm_config.jeita_cv_ibat_limit, g_cp_charging_lmt.cp_jeita_cc / 1000);
    pm_config.bat_volt_lp_lmt = min(pm_config.jeita_cv_vbat_limit, g_cp_charging_lmt.cp_jeita_cv / 1000);
    /*HS07 code for HS07-465 by xiongxiaoliang at 20250408 end*/
    ...
    } else if (vbat_sw_now < pm_config.bat_volt_lp_lmt - 250) {
        ibus_limit = min(pm_config.bus_curr_lp_lmt, pdpm->apdo_max_curr);
        pdpm->ibus_lmt_change_timer = 0;
    } else {
        pdpm->ibus_lmt_change_timer = 0;
    ...
    /*A06 code for AL7160A-830 by jiashixian at 20240525 start*/
    if (vbat_sw_now > pm_config.bat_volt_lp_lmt - BAT_CV_OFFSET) {
        step_vbat = -pm_config.fc2_steps;
    } else if (vbat_sw_now < (pm_config.bat_volt_lp_lmt - (BAT_CV_OFFSET + 7))) {
        step_vbat = pm_config.fc2_steps;
    }
    /*A06 code for AL7160A-830 by jiashixian at 20240525 end*/
    /*HS07 code for HS07-389 by xiongxiaoliang at 20250407 end*/
    ...
    */
    ...
    /*A06 code for AL7160A-830 by jiashixian at 20240525 start*/
    /*HS07 code for HS07-389 by xiongxiaoliang at 20250407 start*/
    if (vbat_sw_now >= pm_config.bat_volt_lp_lmt - (BAT_CV_OFFSET + 7)
    ...
    /*HS07 code for HS07-2413 by xiongxiaoliang at 20250428 start*/
    // request apdo not exceed MAX_REQUEST_POWER
    if (pdpm->request_voltage * pdpm->request_current >= MAX_REQUEST_POWER && steps > 0) {
        steps = 0;
        pr_err("%s step result: (%d)\n", __func__, steps);
    }
    /*HS07 code for HS07-2413 by xiongxiaoliang at 20250428 end*/
...
static int usbpd_pm_sm(struct usbpd_pm *pdpm)
{
    ...
    if ((g_cp_charging_lmt.cp_jeita_stop_chg) ||
      ...
      (g_cp_charging_lmt.cp_ss_stop_chg)  || g_cp_charging_lmt.cp_hv_disable) {
        /*A06 code for AL7160A-1449 by jiashixian at 20240522 start*/
        pr_err("cp stop charge reason:jeita_stop=%d,thermal_stop=%d,fac_stop=%d,ss_stop=%d,hv_disable=%d\n",g_cp_charging_lmt.cp_jeita_stop_chg,g_cp_charging_lmt.cp_thermal_stop_chg,g_cp_charging_lmt.cp_fac_stop_chg,g_cp_charging_lmt.cp_ss_stop_chg,g_cp_charging_lmt.cp_hv_disable);
        /*A06 code for AL7160A-1449 by jiashixian at 20240522 end*/
        recover = true;
...
static void usbpd_pm_remove(struct platform_device *pdev)
{
