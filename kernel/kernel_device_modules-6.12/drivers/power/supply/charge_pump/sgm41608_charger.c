...
static int sgm41608_register_interrupt(struct sgm41608_chip *sgm)
{
    ...
    if (gpio_is_valid(sgm->irq_gpio)) {
        // ret = gpio_request_one(sgm->irq_gpio, GPIOD_IN,"sgm41608_irq");
        ret = gpio_direction_input(sgm->irq_gpio);
        if (ret) {
...
static void sgm41608_charger_remove(struct i2c_client *client)
{
