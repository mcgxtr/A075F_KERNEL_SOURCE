...
/*HS07 code for SR-AL7761A-01-60 by xiongxiaoliang at 20250305 start*/
/*HS07 code for SR-AL7761A-01-154 by yexuedong at 20250312 start*/
#include "mtk_disp_notify.h"
#include <linux/thermal.h>
/*HS07 code for SR-AL7761A-01-154 by yexuedong at 20250312 end*/
...
static struct mtk_charger *gs_pinfo = NULL;
/*HS07 code for SR-AL7761A-01-154 by yexuedong at 20250312 end*/
/*HS07 code for SR-AL7761A-01-60 by xiongxiaoliang at 20250305 end*/

/*HS07 code for P250509-06802 by lina at 20250515 start*/
#if !defined(CONFIG_ODM_CUSTOM_FACTORY_BUILD)
extern bool g_batt_soc_rechg;
#endif//CONFIG_ODM_CUSTOM_FACTORY_BUILD
/*HS07 code for P250509-06802 by lina at 20250515 end*/
	...
	bool en)
{
	...
	if (info->algo.enable_charging != NULL) {
		#if IS_ENABLED(CONFIG_CUSTOM_PROJECT_HS07)
			g_cp_charging_lmt.cp_fac_stop_chg = !en ? 1 : 0;
		#endif
...
extern void usbpd_pm_set_shipmode_adc(int enbale);
#endif
...
int mtk_charger_set_shipmode(struct mtk_charger *info, bool enbale)
{
	...
	do {
		...
		#if defined(CONFIG_CUSTOM_PROJECT_HS07)
		if(enbale == true){
			usbpd_pm_set_shipmode_adc(false);
		}
		#endif
		/*HS07 code for HS07-40 by yexuedong at 20250228 end*/
...
static int read_dt_thermal_array(struct device_node *node,
				const char *prop_name, unsigned int *dest_array,
				int *dest_size)
{
	int ret = 0;
	int count = 0;
	...
	// get thermal array size
	count = of_property_count_elems_of_size(node, prop_name, sizeof(u32));
	if (count <= 0 || count > MAX_THERMAL_CUR_SIZE) {
		pr_err("Invalid %s size: %d (max %d)\n",
			prop_name, count, MAX_THERMAL_CUR_SIZE);
		return -EINVAL;
	}

	// read thermal cur data
	ret = of_property_read_u32_array(node, prop_name, dest_array, count);
	if (ret) {
	...
	*dest_size = count;
	...
	// print thermal current values
	pr_info("Read %d %s values:\n", count, prop_name);
	for (i=0; i < count; i++) {
		pr_info("%s[%d] = %u\n", prop_name, i, dest_array[i]);
	}
...
extern bool gxy_is_auth_battery(void);
static void gxy_auth_battery_cv_config(void)
{
	static struct mtk_charger *s_info = NULL;
	struct power_supply *psy = NULL;
	struct device_node *np = NULL;
	u32 val = 0;
	static int set_count = 2; // max retry count
	...
	if (gxy_is_auth_battery() != true)
	{
		GXY_PSY_ERR("%s: not auth battery, return\n", __func__);
		return;
	}
	...
	if (of_property_read_u32(np, "auth_battery_cv", &val) >= 0)
		s_info->data.battery_cv = val;
	else {
		chr_err("use original BATTERY_CV:%d\n", s_info->data.battery_cv);
	}
	if (of_property_read_u32(np, "auth_jeita_temp_t2_to_t3_cv", &val) >= 0)
		s_info->data.jeita_temp_t2_to_t3_cv = val;
	else {
		chr_err("use original JEITA_TEMP_T2_TO_T3_CV:%d\n",
			s_info->data.jeita_temp_t2_to_t3_cv);
	}
	if (of_property_read_u32(np, "auth_jeita_temp_t1_to_t2_cv", &val) >= 0)
		s_info->data.jeita_temp_t1_to_t2_cv = val;
	else {
		chr_err("use original JEITA_TEMP_T1_TO_T2_CV:%d\n",
			s_info->data.jeita_temp_t1_to_t2_cv);
	}
	if (of_property_read_u32(np, "auth_jeita_temp_t0_to_t1_cv", &val) >= 0)
		s_info->data.jeita_temp_t0_to_t1_cv = val;
	else {
		chr_err("use original JEITA_TEMP_T0_TO_T1_CV:%d\n",
			s_info->data.jeita_temp_t0_to_t1_cv);
	}
	if (of_property_read_u32(np, "auth_jeita_temp_below_t0_cv", &val) >= 0)
		s_info->data.jeita_temp_below_t0_cv = val;
	else {
		chr_err("use original JEITA_TEMP_BELOW_T0_CV:%d\n",
			s_info->data.jeita_temp_below_t0_cv);
	}
#if !defined(CONFIG_ODM_CUSTOM_FACTORY_BUILD)
	if (read_range_data_from_node(np, "auth_gxy,cv-ranges",
		s_info->data.batt_cv_data,
		MAX_CYCLE_COUNT, s_info->data.battery_cv)) {
		pr_err("Read gxy,cv-ranges failed from dtsi\n");
	}
#endif //!CONFIG_ODM_CUSTOM_FACTORY_BUILD
#ifdef CONFIG_ODM_CUSTOM_D85_BUILD
	s_info->data.battery_cv = D85_BATTERY_CV;
	s_info->data.jeita_temp_above_t4_cv = D85_JEITA_TEMP_CV;
	s_info->data.jeita_temp_t3_to_t4_cv = D85_JEITA_TEMP_CV;
	s_info->data.jeita_temp_t2_to_t3_cv = D85_JEITA_TEMP_CV;
	s_info->data.jeita_temp_t1_to_t2_cv = D85_JEITA_TEMP_CV;
	s_info->data.jeita_temp_t0_to_t1_cv = D85_JEITA_TEMP_CV;
#endif
	GXY_PSY_ERR("%s: auth battery, reset cv to %d\n", __func__, s_info->data.battery_cv);
}
				...
				struct device *dev)
{
	...
	#if defined(CONFIG_CUSTOM_PROJECT_OT11) || defined(CONFIG_CUSTOM_PROJECT_HS07) || defined(CONFIG_CUSTOM_PROJECT_HST11)
	...
	info->enable_custom_thermal_buck = of_property_read_bool(np, "enable_custom_thermal_buck");
	rc = read_dt_thermal_array(np, "thermal_lcd_on_buck_cur",
			info->data.thermal_lcd_on_buck_cur,
			&info->data.thermal_buck_cur_size);
	if (rc < 0) {
		pr_err("Read thermal_lcd_on_buck_cur failed from dtsi, rc=%d\n", rc);
		info->enable_custom_thermal_buck = false;
	}
	rc = read_dt_thermal_array(np, "thermal_lcd_off_buck_cur",
			info->data.thermal_lcd_off_buck_cur,
			&info->data.thermal_buck_cur_size);
	if (rc < 0) {
		pr_err("Read thermal_lcd_off_buck_cur failed from dtsi, rc=%d\n", rc);
		info->enable_custom_thermal_buck = false;
	}
	/* HST11 code for AX7800A-494 by wenyaqi at 20250424 end */
	...
	/* HST11 code for AX7800A-632 by wenyaqi at 20250512 start */
	info->support_auth_ic = of_property_read_bool(np, "support_auth_ic");
	/* HST11 code for AX7800A-632 by wenyaqi at 20250512 end */
...
static int get_ap_hw_temp(void)
{
	...
	int temperature = 0;
	int ret = 0;
	thermal_dev = thermal_zone_get_zone_by_name("ap_ntc");
	if (IS_ERR(thermal_dev)) {
		chr_err("%s: Unable to get thermal zone for ap_ntc\n", __func__);
		goto AP_THERM_FAIL;
	}
	...
	if (ret) {
		...
		goto AP_THERM_FAIL;
	/*HS07 code for SR-AL7761A-01-154 by yexuedong at 20250317 start*/
	} else {
		temperature /= 1000;
		chr_err("%s: get ap_thermal temperature = %d", __func__, temperature);
		return temperature;
	}
	/*HS07 code for SR-AL7761A-01-154 by yexuedong at 20250317 end*/
AP_THERM_FAIL:
...
void ap_thermal_machine_to_cp(struct mtk_charger *info)
{
	...
	if (!info->lcm_sts) {
		...
		if (cp_lcmon_data->sm == TEMP_T3_TO_T4) {
			...
			/*HS07 code for HS07-355|HS07-503|HS07-3232 by yexuedong at 20250519 start*/
			...
			g_cp_charging_lmt.cp_thermal_stop_chg = 1;
			#endif
			/*HS07 code for HS07-355|HS07-503|HS07-3232 by yexuedong at 20250519 end*/
...
/*HS07  code for SR-AL7761A-01-98  by huofudong at 20240407 start*/
...
static void gxy_set_slate_mode(struct mtk_charger *info, int slate_mode)
{
	switch (info->gxy_tcpc_info) {
		...
		#if defined(CONFIG_CUSTOM_PROJECT_HS07) || defined(CONFIG_CUSTOM_PROJECT_HST11)
		case GXY_BAT_TCPC_INFO_UPM6922P:
		case GXY_BAT_TCPC_INFO_SCV89601P:
		#endif
...
/*HS07  code for SR-AL7761A-01-98  by huofudong at 20240407 end*/
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	switch (alg->alg_id) {
#if defined(CONFIG_CUSTOM_PROJECT_OT11) || defined(CONFIG_CUSTOM_PROJECT_HST11) || defined(CONFIG_CUSTOM_PROJECT_HS07)
	case AFC_ID:
		alg_name = "AFC";
		break;
	case PDC_ID:
...
static void charger_check_status(struct mtk_charger *info)
{
	...
	if (info->gxy_batt_soc_rechg) {
		...
		/*HS07 code for P250709-05486 by xiongxiaoliang at 20250709 start*/
		} else if (uisoc == 100 && bat_status.intval == POWER_SUPPLY_STATUS_FULL) {
		/*HS07 code for P250709-05486 by xiongxiaoliang at 20250709 end*/
			...
			g_batt_soc_rechg = info->gxy_batt_soc_rechg_flag;
...
/*HS07 code for HS07-4806 by xiongxiaoliang at 20250630 start*/
#if IS_ENABLED(CONFIG_CUSTOM_PROJECT_HS07)
static int gxy_check_typec_attached_snk(struct tcpc_device *tcpc)
{
    u8 type = 0;

    if (!tcpc) {
        GXY_PSY_ERR("tcpc is null return\n");
        return -EINVAL;
    }

    type = tcpm_inquire_typec_attach_state(tcpc);
    if (type != TYPEC_ATTACHED_SNK) {
        GXY_PSY_ERR("no typec_attached_snk, type:%d\n", type);
        return -EINVAL;
    }
...
int gxy_get_pps_apdo(struct mtk_charger *info)
{
    ...
    if (gxy_check_typec_attached_snk(tcpc) < 0)
        return 0;

    if (!tcpm_inquire_pd_pe_ready(tcpc)) {
        GXY_PSY_ERR("PD PE not ready\n");
        return 0;
    }
    ...
    while (1) {
        ...
        if (ret != TCP_DPM_RET_SUCCESS) {
            GXY_PSY_ERR("inquire pd apdo fail(%d)\n", ret);
            break;
        }

        pwr_cur = apdo_cap.max_mv * apdo_cap.ma / 1000;
        if (info->pwr_max < pwr_cur) {
            info->pwr_max = pwr_cur;
        }
        GXY_PSY_ERR("cap_idx[%d], %d mv ~ %d mv, %d ma, pwr_cur:%d, pwr_max:%d\n",
            cap_idx, apdo_cap.min_mv, apdo_cap.max_mv, apdo_cap.ma, pwr_cur, info->pwr_max);
    }

    return 0;
}
EXPORT_SYMBOL(gxy_get_pps_apdo);
#endif
/*HS07 code for HS07-4806 by xiongxiaoliang at 20250630 end*/
				...
				int chr_type)
{
	...
	/*Tab A9 code for SR-AX6739A-01-486 by qiaodan at 20230512 end*/
	/*HS07 code for HS07-264 by xiongxiaoliang at 20250401 start*/
	#if IS_ENABLED(CONFIG_CUSTOM_PROJECT_HS07)
	g_cp_charging_lmt.cp_fac_stop_chg = 0;
	#endif
	/*HS07 code for HS07-264 by xiongxiaoliang at 20250401 end*/
...
static void kpoc_power_off_check(struct mtk_charger *info)
{
	...
	/* HST11 code for P250523-09250 by wenyaqi at 20250603 start */
	#ifndef CONFIG_ODM_CUSTOM_BUILD
	chr_err("%s, sep, not shutdown in kernel\n", __func__);
	return;
	#endif
	/* HST11 code for P250523-09250 by wenyaqi at 20250603 end */
...
static int charger_routine_thread(void *arg)
{
	...
	while (1) {
		...
		/* HST11 code for AX7800A-632 by wenyaqi at 20250512 start */
		if (info->support_auth_ic) {
			gxy_auth_battery_cv_config();
		}
		/* HST11 code for AX7800A-632 by wenyaqi at 20250512 end */
			...
			const union power_supply_propval *val)
{
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		/*Tab A9_v code for P250221-02691 by shanxinkai at 20250224 start*/
		if (val->intval > 0 && (!info->hv_disable_flag))
			info->enable_hv_charging = true;
		else
			info->enable_hv_charging = false;
		break;
		/*Tab A9_v code for P250221-02691 by shanxinkai at 20250224 end*/
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		info->chg_data[idx].thermal_charging_current_limit =
...
static void lcm_sts_switch(bool onoff)
{
	...
	/* onoff = 1: LCM ON */
	if (onoff) {
		gs_pinfo->lcm_sts = true;
	} else {
		gs_pinfo->lcm_sts = false;
	}
	chr_err("%s: onoff[%d], lcm_sts[%d]\n", __func__, onoff, gs_pinfo->lcm_sts);
}
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	info->ap_temp = 0xffff;
	#endif
	if (info->enable_custom_thermal_buck) {
		gs_pinfo = info;
