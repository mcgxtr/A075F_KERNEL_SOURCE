...
struct mtk_pd_adapter_info {
	...
	int *pd_type;
/* A07 LTE_BOS code for AL7761C-1 by xiaojian at 20250801 start */
#if defined(CONFIG_CUSTOM_PROJECT_OT11) || defined(CONFIG_CUSTOM_PROJECT_HS07) || defined(CONFIG_CUSTOM_PROJECT_HST11)
/* A07 LTE_BOS code for AL7761C-1 by xiaojian at 20250801 end */
...
/* Tab A9_V code for AL6739VDEV-13 by zhangziyi at 20240925 start */
static int __maybe_unused pd_adapter_enable_power_path(bool en)
{
/* HS07 code for HS07-1480 by lina at 20250425 start */
#if defined(CONFIG_CUSTOM_PROJECT_OT11) || defined(CONFIG_CUSTOM_PROJECT_HST11) || defined(CONFIG_CUSTOM_PROJECT_HS07)
/* HS07 code for HS07-1480 by lina at 20250425 end */
	...
	charger_dev_set_vindpm(chg_dev, en);
	#else
...
/* A07 LTE_BOS code for AL7761C-1 by xiaojian at 20250801 end */
	...
	/* HS07 code for HS07-1480 by lina at 20250425 start */
	#if IS_ENABLED(CONFIG_CUSTOM_USBIF)
	static bool set_vindpm_sts = false;
	#endif//CONFIG_CUSTOM_USBIF
	/* HS07 code for HS07-1480 by lina at 20250425 end */
...
/* Tab A9_V code for AL6739VDEV-13|AL6739VDEV-18 by zhangziyi at 20241021 end */
		...
		if (noti->vbus_state.type & TCP_VBUS_CTRL_PD_DETECT) {
			if (sink_mv && !sink_ma) {
				if (set_vindpm_sts == false) {
					pd_adapter_enable_power_path(true);
					set_vindpm_sts = true;
					pr_err("%s  will set vindpm 10500~10700mv\n", __func__);
				}
			} else {
				if (set_vindpm_sts == true) {
					pd_adapter_enable_power_path(false);
					set_vindpm_sts = false;
					pr_err("%s will set vindpm 4600mv\n", __func__);
				}
			}
		}
		#else
