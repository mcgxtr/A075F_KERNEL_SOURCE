...
/* HST11_BOS code for AX7800W-22 by gaochao at 20250909 start */
...
int power_get_battery_ocv(void)
{
    ...
    ocv = gauge_get_int_property(gauge->gm, GAUGE_PROP_ZCV);

    return ocv;
}
EXPORT_SYMBOL(power_get_battery_ocv);
/* HST11_BOS code for AX7800W-22 by gaochao at 20250909 end */
...
int power_get_sys_battery_current(int *p_curr_now, int *p_curr_avg, int select)
{
    int ret_curr_now = 0;
    int ret_curr_avg = 0;
    int curr_now = 0; // instant current
    int curr_avg = 0; // average current
    static struct power_supply *psy = NULL;
    struct mtk_gauge *gauge = NULL;
    // struct mtk_battery *gm = NULL;

    if (!p_curr_now || !p_curr_avg) {
        pr_err("[%s][%d]p_curr_now or p_curr_avg is NULL\n", __FUNCTION__, __LINE__);
        return -1;
    }
    ...
    if (select == 0) {
        ret_curr_now = gauge_get_property(gauge->gm, GAUGE_PROP_BATTERY_CURRENT, &curr_now);
        if (ret_curr_now) {
            pr_err("[%s][%d]Failed to get IBAT, ret_curr_now=%d\n", __FUNCTION__, __LINE__, ret_curr_now);
            curr_now = gauge->gm->ibat;
        }
        *p_curr_now = curr_now * 100;
        pr_debug("[%s][%d]curr_now=%d 0.1mA, curr_now * 100=%duA, %d\n", __FUNCTION__, __LINE__,
                curr_now, curr_now * 100, ret_curr_now);
        return ret_curr_now;
    }
    if (select == 1) {
        ret_curr_avg = gauge_get_property(gauge->gm, GAUGE_PROP_AVERAGE_CURRENT, &curr_avg);
        if (ret_curr_avg) {
            pr_err("[%s][%d]Failed to get IBAT, ret_curr_avg=%d\n", __FUNCTION__, __LINE__, ret_curr_avg);
            curr_avg = gauge->gm->ibat;
        }
        *p_curr_avg = curr_avg * 100;
        pr_debug("[%s][%d]curr_avg=%d 0.1mA, curr_avg * 100=%duA, %d\n", __FUNCTION__, __LINE__,
                curr_avg, curr_avg * 100, ret_curr_avg);
        return ret_curr_avg;
    }

    return 0;
}
EXPORT_SYMBOL(power_get_sys_battery_current);
...
static void mt6358_gauge_remove(struct platform_device *pdev)
{
