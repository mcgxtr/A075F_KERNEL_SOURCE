...
static void sec_auth_poll_work_func(struct work_struct *work)
{
    ...
    if (psy == NULL) {
        ...
        if (psy == NULL) {
            pr_info("%s: get battery psy failed\n", __func__);
            return;
        }
    }
	power_supply_get_property(psy, POWER_SUPPLY_PROP_STATUS, &value);
...
static int authon_driver_parse_dt(struct authon_driver_platform_data *pdata, struct authon_driver_data *authon_driver)
{
	...
	if (IS_ERR_OR_NULL(np) || IS_ERR_OR_NULL(authon_driver)) {
		pr_info("%s: np/authon_driver is err or null\n", __func__);
		return -EINVAL;
	} else {
#if !defined(CONFIG_SEC_FACTORY)
...
#endif
		pdata->swi_gpio_cnt = gpiod_count(authon_driver->dev,
			"authon,swi");
		if (pdata->swi_gpio_cnt > 0) {
			...
			for (i = 0; i < pdata->swi_gpio_cnt; i++) {
				pdata->swi_gpio[i] = of_get_named_gpio(np, "authon,swi-gpios", i);
...
// convert year to alphabet
static char convertYear(const char *yearStr) {
...
// convert month to number(0x)
static char convertMonth(const char *monthStr) {
...
static int convert_qr_code(char *temp_buf, char *std_data)
{
	char year_str[3] = {temp_buf[18], temp_buf[19], '\0'};
	char month_str[3] = {temp_buf[20], temp_buf[21], '\0'};

	// 1. product code[11]
	memcpy(std_data, temp_buf + 2, 11);
	// 2. +[1]
	std_data[11] = '+';
	// 3. Pack vendor[1]
	if (temp_buf[16] == 'F' && temp_buf[17] == 'M') {
		std_data[12] = 'F';
	} else {
		std_data[12] = '0';
	}
	// 4. Cellvendor[1]
	if (temp_buf[26] == 'A' && temp_buf[27] == 'T') {
		std_data[13] = 'L';
	} else {
		std_data[13] = '0';
	}
	...
	std_data[14] = '1';
	// 6. Year[1]
	std_data[15] = convertYear(year_str);
	// 7. Month[1]
	std_data[16] = convertMonth(month_str);
	// 8. Day[2]
	std_data[17] = temp_buf[22];
	std_data[18] = temp_buf[23];
	// 9. Line[1]
	std_data[19] = temp_buf[25];
	// 10. Vendor[1]
	std_data[20] = 'K';
	// 11. +[1]
	std_data[21] = '+';
	// 12. SN[5]
	memcpy(std_data + 22, temp_buf + 29, 5);
	// 13. Cell Site[1]
	std_data[27] = 'N';
...
static int get_qr_code(char *data)
{
	...
	err = read_from_page_buffer(QR_CODE_PAGE,
			QR_CODE_START_BYTE, QR_CODE_BYTE_CNT, temp_buf);
	if (err != 0) {
		mutex_unlock(&g_auth_driv->sec_auth_mutex);
        return err;
    }
	convert_qr_code(temp_buf, std_data);
	strncpy(data, std_data, sizeof(std_data) - 1);
	data[sizeof(std_data) - 1] = '\0';
	mutex_unlock(&g_auth_driv->sec_auth_mutex);
	return err;
}
...
/* HST11 code for P250719-00216 by wenyaqi at 20250722 start */
extern int g_board_id_status;
static bool auth_area_pcbainfo_judge(void)
{
    int board_id = 0;

    board_id = g_board_id_status & 0x07;
    pr_info("%s [%d, %d]", __func__, g_board_id_status, board_id);

    if (board_id == 0x00 || board_id == 0x04 || board_id == 0x06) {
        return true;
    }
    return false;
}
/* HST11 code for P250719-00216 by wenyaqi at 20250722 end */
...
static int authon_probe(struct platform_device *pdev)
{
	...
	/* HST11 code for P250719-00216 by wenyaqi at 20250722 start */
	if (!auth_area_pcbainfo_judge()) {
		pr_info("Not in the European region, no need to run authon_probe\n");
		return  -ENODEV;
	}
	/* HST11 code for P250719-00216 by wenyaqi at 20250722 end */
/***************************************************************************
 ...
 *************************************************************************/
	...
	ret = authon_driver_parse_dt(authon_driver->pdata, authon_driver);
	if (ret < 0) {
...
static void authon_remove(struct platform_device *pdev)
{
