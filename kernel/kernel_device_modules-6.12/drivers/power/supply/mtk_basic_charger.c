...
static void gxy_select_thermal_buck_current(struct mtk_charger *info, struct charger_data *pdata)
{
	int match_index = -1;
	int i = 0;

	if (pdata->thermal_charging_current_limit != -1) {
		for (i = 0; i < info->data.thermal_buck_cur_size; i++) {
			if (pdata->thermal_charging_current_limit == info->data.thermal_lcd_on_buck_cur[i]){
				match_index = i;
				break;
			}
		}
		...
		if (match_index == -1) {
			match_index = info->data.thermal_buck_cur_size - 1;
		}

		if (pdata->thermal_charging_current_limit <=
			pdata->charging_current_limit) {
			pdata->charging_current_limit = info->lcm_sts ?
				info->data.thermal_lcd_on_buck_cur[match_index] :
				info->data.thermal_lcd_off_buck_cur[match_index];
			info->setting.charging_current_limit1 = info->lcm_sts ?
				info->data.thermal_lcd_on_buck_cur[match_index] :
				info->data.thermal_lcd_off_buck_cur[match_index];
		}
	}

	pr_err("%s:lcm_sts[%d]match[%d]cur[%d %d]raw_th_cur[%d]\n", __func__, info->lcm_sts, match_index,
		info->setting.charging_current_limit1, pdata->charging_current_limit, pdata->thermal_charging_current_limit);
}
	...
	struct chg_limit_setting *setting)
{
	...
	if(info->enable_custom_thermal_buck) {
		gxy_select_thermal_buck_current(info, pdata);
	} else {
		/*Tab A9 code for SR-AX6739A-01-504 by qiaodan at 20230531 start*/
...
static int do_algorithm(struct mtk_charger *info)
{
	...
	#if defined(CONFIG_CUSTOM_PROJECT_HS07)
	g_cp_charging_lmt.cp_hv_disable = !info->enable_hv_charging;
	#endif
	...
	if (info->chg1_dev != NULL) {
		...
		/* HS07 code for P251008-01019 by xiaozhenting at 20251016 start */
		#if 0
		charger_dev_kick_wdt(info->chg1_dev);
		#endif
		/* HS07 code for P251008-01019 by xiaozhenting at 20251016 end */
	}
