...
static int vbus_ctrl_probe(struct platform_device *pdev)
{
    ...
    if (ret < 0) {
        ...
        /*HS07 code for HS07-2562 by xiongxiaoliang at 20250430 start*/
        ret = gpio_request(g_vbus_ctrl_t->vbus_ctrl_gpio, "usb_mos_gpio");
        /*HS07 code for HS07-2562 by xiongxiaoliang at 20250430 end*/
        if (ret) {
            pr_err("[%s] failed to request %d\n", __func__, g_vbus_ctrl_t->vbus_ctrl_gpio);
            return -EINVAL;
        }
    }
...
static void vbus_ctrl_remove(struct platform_device *dev)
{
