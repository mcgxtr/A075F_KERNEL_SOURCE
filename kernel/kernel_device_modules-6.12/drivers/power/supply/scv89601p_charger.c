...
#define SCV89601P_DRV_VERSION         "1.7"
...
struct scv89601p_chip {
    ...
    /* HST11 code for AX7800A-4393 by zhangziyi at 20250827 start */
    #if defined(CONFIG_CUSTOM_PROJECT_HST11)
    struct i2c_adapter *adp;
    #endif
    /* HST11 code for AX7800A-4393 by zhangziyi at 20250827 end */
#ifdef CONFIG_MTK_CLASS
...
static int scv89601p_hk_buck_irq_mask(struct scv89601p_chip *sc)
{
    ...
    /* A07 code for AL7761C-1 by lina at 20250805 start */
    ret |= scv89601p_field_write(sc, F_QB_EN_MASK, 1);
    /* A07 code for AL7761C-1 by lina at 20250805 end */
...
__maybe_unused
static int scv89601p_set_otg_hiccup_protection(struct scv89601p_chip *sc)
{
    int ret = 0;
    uint8_t otg_hiccup_data[] = {0xB0, 0x12};

    struct i2c_msg msg = {
        .addr = 0x64,
        .flags = 0,
    };
    msg.len = sizeof(otg_hiccup_data);
    msg.buf = otg_hiccup_data;
    ret = i2c_transfer(sc->adapter, &msg, 1);
...
static int scv89601p_set_hiz(struct scv89601p_chip *sc, bool enable)
{
    int reg_val = enable ? 1 : 0;
    int read_reg_val = 0;
    int ret = scv89601p_field_read(sc, F_HIZ_EN, &read_reg_val);
    if (ret < 0) {
...
#if defined(CONFIG_CUSTOM_PROJECT_HST11)
__maybe_unused
static int scv89601p_pd_reset(struct scv89601p_chip *sc)
{
    int ret = 0;
    uint8_t reset[] = {0xA0, 0x01};
    struct i2c_msg msg = {
        .addr = 0x62,
        .flags = 0,
    };
...
static int scv89601p_get_shutdown(struct scv89601p_chip *sc)
{
    int ret = 0;
    unsigned char shutdown_en;
    unsigned char shutdown_reg = 0x9B;
    struct i2c_msg xfer[2];
    xfer[0].addr    = 0x62;
    xfer[0].flags   = 0;
    xfer[0].len     = 1;
    xfer[0].buf     = &shutdown_reg;
    xfer[1].addr    = 0x62;
    xfer[1].flags   = I2C_M_RD;
    xfer[1].len     = 1;
    xfer[1].buf     = &shutdown_en;
    ret = i2c_transfer(sc->adp, xfer, 2);
    if (ret < 0) {
        pr_info("Error: %d\n", ret);
    }
    pr_info("scv89601p_get_shutdown: %x\n", shutdown_en);
    return ret;
}
#endif
/* HST11 code for AX7800A-4393 by zhangziyi at 20250827 end */
...
__maybe_unused
static int scv89601p_check_otg_enabled(struct scv89601p_chip *sc, bool *enable)
{
    int ret, reg_val;
...
static int scv89601p_init_device(struct scv89601p_chip *sc) {
    ...
    scv89601p_set_otg_hiccup_protection(sc);
...
static DEVICE_ATTR(registers, 0664, scv89601p_show_registers,
                scv89601p_store_register);
static DEVICE_ATTR(scv89601p_test, 0664, NULL,
...
static int scv89601p_chg_get_property(struct power_supply *psy,
    ...
    case POWER_SUPPLY_PROP_TYPE:
        /*Tab A11_Bos code for AX7800W-11 by xizozhenting at 20250711 start*/
        val->intval = sc->chg_type;
        /*Tab A11_Bos code for AX7800W-11 by xizozhenting at 20250711 end*/
        break;
    default:
        ret = -EINVAL;
        break;
...
static int scv89601p_charger_probe(struct i2c_client *client)
{
    ...
    int reg_val = 0;
    /*Tab A9 code for AX6739A-2077 | AX6739A-2728 by wenyaqi at 20230906 end*/
    const struct i2c_device_id __maybe_unused *id;
...
static void scv89601p_charger_remove(struct i2c_client *client)
{
 ...
 * 1.1
 * (1) add regn bypass function
 * 1.2
 * (1) add otg hiccup protection
 * 1.3
 * (1) change vbat adc LSB from 18mV to 24mV
 * 1.4
 * (1) change VBUSGOOD FLAG from vbus present to vbus good
 * 1.5
 * (1) mask CH_FLG irq
 * (2) reduce dump reg number
 * 1.6
 * (1) change the insert and remove irq condition
 * 1.7
 * (1) Turn off VINDPM_TRACK function. Resolve the issue of the current being
 *     very low during the rapid full charging stage.
 */
