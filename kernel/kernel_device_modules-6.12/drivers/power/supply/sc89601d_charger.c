...
static int sc8960x_get_charge_stat(struct sc8960x_chip *sc)
{
    ...
    dev_info(sc->dev, "%s s_psy_stat_old=%d,psy_stat_now=%d\n", __func__, s_psy_stat_old, psy_stat_now);
    return psy_stat_now;
}
...
static int sc8960x_get_charger_type(struct sc8960x_chip *sc)
{
    ...
    dev_info(sc->dev, "%s vbus stat: 0x%02x, %d\n", __func__, reg_val, sc->chg_type);
...
static irqreturn_t sc8960x_irq_handler(int irq, void *data)
{
    ...
    dev_info(sc->dev, "%s: prev_vbus_gd: %d\n", __func__, prev_vbus_gd);
    dev_info(sc->dev, "%s: sc->vbus_good: %d\n", __func__, sc->vbus_good);
...
static int sc8960x_irq_register(struct sc8960x_chip *sc)
{
    ...
    ret = gpio_request_one(sc->irq_gpio, GPIOD_IN, "sc8960x_irq");
    if (ret) {
