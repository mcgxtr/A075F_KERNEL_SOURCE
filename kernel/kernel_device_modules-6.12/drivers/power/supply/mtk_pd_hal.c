...
int pd_hal_is_adapter_ready(struct chg_alg_device *alg)
{
	...
	if (chg_psy == NULL || IS_ERR(chg_psy))
		...
		if (info->select_adapter) {
			...
			/*A11_W code for AW7800W-11 by xiaozhenting at 20250708 start*/
			#if IS_ENABLED(CONFIG_CUSTOM_PROJECT_OT11) || defined(CONFIG_CUSTOM_PROJECT_HST11)
			if ((info->ta_capability == APDO_TA) || (info->ta_capability == WO_APDO_TA))
			/*HS07 code for HS07-166 by xiongxiaoliang at 20250312 start*/
			#elif IS_ENABLED(CONFIG_CUSTOM_PROJECT_HS07)
			if (info->ta_capability == WO_APDO_TA)
			/*HS07 code for HS07-166 by xiongxiaoliang at 20250312 end*/
			#else
			if (info->ta_capability == APDO_TA)
			#endif
			/*A11_W code for AW7800W-11 by xiaozhenting at 20250708 end*/
	...
	enum chg_idx chgidx, u32 ua)
{
	...
	if (ua > PD_IBUS_LOW_BOUND) {
		...
		#if defined(CONFIG_CUSTOM_PROJECT_OT11_NA) || defined(CONFIG_CUSTOM_PROJECT_HS07) || defined(CONFIG_CUSTOM_PROJECT_HST11)
