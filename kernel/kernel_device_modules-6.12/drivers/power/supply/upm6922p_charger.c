...
#define pr_fmt(fmt) "[upm6922p-charger]:%s:" fmt, __func__
...
struct upm6922 {
    ...
    int psy_usb_type;
    /*HS07 code for HS07-44 by xiongxiaoliang at 20250228 start*/
    int psy_type;
    /*HS07 code for HS07-44 by xiongxiaoliang at 20250228 end*/
    /*HS07 code for HS07-48 by xiongxiaoliang at 20250303 start*/
    bool is_recharging;
    /*HS07 code for HS07-48 by xiongxiaoliang at 20250303 end*/
...
static int upm6922_enable_hiz_mode(struct upm6922 *upm)
{
    ...
    /*A06_V code for AL7160AV-50 by yexuedong at 20241114 start*/
    upm->hvdcp_done = false;
    /*A06_V code for AL7160AV-50 by yexuedong at 20241114 end*/
...
/*HS07 code for HS07-752 by xiongxiaoliang at 20250415 start*/
static int upm6922_enable_hz(struct charger_device *chg_dev, bool enable)
{
    struct upm6922 *upm = charger_get_data(chg_dev);

    pr_info("%s value = %d\n", __func__, enable);
    if (enable) {
        upm6922_enable_hiz_mode(upm);
    } else {
        upm6922_disable_hiz_mode(upm);
    }
...
static int __maybe_unused upm6922_get_chargevolt(struct upm6922 *upm, int *mv)
{
    ...
    /*HS07 code for HS07-465 by xiongxiaoliang at 20250408 start*/
    cv = ((val1 & REG04_VREG_MASK) >> REG04_VREG_SHIFT) *
                REG04_VREG_LSB + REG04_VREG_BASE;
    /*HS07 code for HS07-465 by xiongxiaoliang at 20250408 end*/
...
static int upm6922_enable_port_charging(struct charger_device *chg_dev, bool enable)
{
    ...
    if (enable) {
        ...
        upm->is_recharging = true;
    } else {
        ret = upm6922_enable_hiz_mode(upm);
        upm->is_recharging = false;
    }
    /*HS07 code for HS07-48 by xiongxiaoliang at 20250303 end*/
    return ret;
}
...
/*HS07 code for HS07-2435 by xiongxiaoliang at 20250429 start*/
static int upm6922_get_input_volt_limit(struct upm6922 *upm, int *mv)
{
    u8 val1, val2;
    int ret = 0;
    int vindpm_os;
    int vindpm;
    int vindpm_os_array[4] = {3900, 5900, 7500, 10500};

    ret = upm6922_read_byte(upm, UPM6922_REG_0D, &val1);
    if (ret < 0){
        pr_err("read UPM6922_REG_0D fail, ret:%d\n", ret);
        return ret;
    }

    ret = upm6922_read_byte(upm, UPM6922_REG_06, &val2);
    if (ret < 0){
        pr_err("read UPM6922_REG_06 fail, ret:%d\n", ret);
        return ret;
    }

    vindpm_os = (val1 & REG0D_VINDPM_OS_MASK) >> REG0D_VINDPM_OS_SHIFT;
    vindpm = ((val2 & REG06_VINDPM_MASK) >> REG06_VINDPM_SHIFT) * REG06_VINDPM_LSB;

    *mv = vindpm_os_array[vindpm_os] + vindpm;

    pr_info("vindpm_os:%d, vindpm:%d\n", vindpm_os, vindpm);
...
void upm6922_set_input_volt_lim(struct upm6922 *upm, bool enable)
{
    ...
    if (upm != NULL) {
        upm6922_set_input_volt_limit(upm, vindpm);
    } else {
        pr_err("[%s] upm is NULL", __func__);
    }
}
/* HS07 code for HS07-1480 by lina at 20250425 end */

void upm6922_set_input_volt_lim_pd(struct upm6922 *upm, bool enable)
{
    unsigned int vindpm = 0;
    if (enable == true) {
        vindpm = 4600;
...
void upm6922_set_input_volt_limit_plugin(struct upm6922 *upm)
{
    if (upm != NULL) {
        upm6922_set_input_volt_lim_pd(upm, false);
...
static void upm6922_delay_set_input_volt_lim(struct work_struct *work)
{
    ...
    upm = container_of(set_input_volt_lim_work, struct upm6922,
    ...
    upm->is_first_plugin = false;
    upm6922_set_input_volt_lim_pd(upm, true);
    pr_err("%s end\n", __func__);
}

/* HS07 code for HS07-1480 by lina at 20250425 start */
static int upm6922_set_vindpm(struct charger_device *chg_dev, bool enable)
{
    struct upm6922 *upm = charger_get_data(chg_dev);
    /*HS07 code for HS07-752 by xiongxiaoliang at 20250514 start*/
    upm->pd_set_vindpm_sts = enable;
    pr_info("%s value = %d, pd_set_vindpm_sts = %d\n", __func__, enable, upm->pd_set_vindpm_sts);
    /*HS07 code for HS07-752 by xiongxiaoliang at 20250514 end*/
    if (enable) {
        upm6922_set_input_volt_lim(upm, false);
    } else {
        upm6922_set_input_volt_lim(upm, true);
    }
...
static int upm6922_get_charging_stat(struct upm6922 *upm, int *stat)
{
    ...
    /*HS07 code for HS07-3330 by xiongxiaoliang at 20250522 start*/
    pr_err("chrg_stat=%d, *stat=%d, chrg_type=%d, chg_config=%d\n", chrg_stat, *stat, upm->chrg_type, upm->chg_config);
    /*HS07 code for HS07-3330 by xiongxiaoliang at 20250522 end*/
...
static int __maybe_unused upm6922_auto_bc12_en(struct upm6922 *upm, bool en)
{
    ...
    if (en) {
        val = REG0C_AUTO_BC12_ENABLE;
    } else {
        val = REG0C_AUTO_BC12_DISABLE;
    }

    return upm6922_update_bits(upm, UPM6922_REG_0C, REG0C_AUTO_BC12_MASK,
                   val << REG0C_AUTO_BC12_SHIFT);
}
...
void upm6922_dump_regs(struct upm6922 *upm)
{
    ...
    /*HS07 code for P250605-02174 by xiongxiaoliang at 20250611 start*/
    ...
    ret = upm6922_read_byte(upm, UPM6922_REG_15, &val);
    if (ret == 0)
        pr_err("Reg[15] = 0x%.2x\n", val);
    /*HS07 code for P250605-02174 by xiongxiaoliang at 20250611 end*/
}
...
static int upm6922_get_chgtype(struct upm6922 *upm, int *type)
{
    ...
    /*HS07 code for HS07-44|HS07-48|HS07-793 by xiongxiaoliang at 20250417 start*/
    if (upm->psy_type == POWER_SUPPLY_TYPE_USB_DCP && upm->hvdcp_done != true) {
    /*HS07 code for HS07-44|HS07-48|HS07-793 by xiongxiaoliang at 20250417 end*/
        /*HS07 code for HS07-628 by xiongxiaoliang at 20250411 start*/
        ret = upm6922_set_input_current_limit(upm, 500);
        if (ret) {
            pr_err("Failed to set iindpm, ret = %d\n", ret);
        }
        /*HS07 code for HS07-628 by xiongxiaoliang at 20250411 end*/
...
static void charger_detect_work_func(struct work_struct *work)
{
    ...
    if (!vbus_attach_pre && upm->vbus_attach) {
        ...
        upm->chg_config = !gxy_check_batt_protection_status();
        #endif
        /*HS07 code for HS07-3330 by xiongxiaoliang at 20250522 end*/
        /*HS07 code for HS07-752 by xiongxiaoliang at 20250415 start*/
        #if IS_ENABLED(CONFIG_CUSTOM_USBIF)
        if (upm->bootmode == 0) {
            upm6922_set_input_volt_limit_plugin(upm);
        }
        #endif
        /*HS07 code for HS07-752 by xiongxiaoliang at 20250415 end*/
    ...
    /*HS07 code for HS07-48 by xiongxiaoliang at 20250303 start*/
    if ((upm->force_dpdm == true || upm->is_recharging == true) && (upm->power_good == true)) {
...
static int upm6922_register_interrupt(struct upm6922 *upm)
{
    ...
    ret = devm_gpio_request_one(upm->dev, upm->intr_gpio, GPIOD_IN,
...
/*HS07 code for HS07-3330 by xiongxiaoliang at 20250522 start*/
static int upm6922_plug_charging(struct charger_device *chg_dev, bool enable)
{
    struct upm6922 *upm = dev_get_drvdata(&chg_dev->dev);
    int ret = 0;
...
static int upm6922_ops_plug_in(struct charger_device *chg_dev)
{
    ...
    ret = upm6922_plug_charging(chg_dev, true);
    if (ret) {
...
static int upm6922_ops_plug_out(struct charger_device *chg_dev)
{
    ...
    ret = upm6922_plug_charging(chg_dev, false);
    if (ret) {
...
static int upm6922_ops_set_ivl(struct charger_device *chg_dev, u32 uv)
{
	...
	/*HS07 code for HS07-2435 by xiongxiaoliang at 20250429 start*/
	pr_info("set vindpm:%d\n", uv);
	/*HS07 code for HS07-2435 by xiongxiaoliang at 20250429 end*/
	return upm6922_set_input_volt_limit(upm, uv / 1000);
}

/*HS07 code for HS07-2435 by xiongxiaoliang at 20250429 start*/
static int upm6922_ops_get_ivl(struct charger_device *chg_dev, u32 *uv)
{
	struct upm6922 *upm = dev_get_drvdata(&chg_dev->dev);
	int mv;
	int ret;

	ret = upm6922_get_input_volt_limit(upm, &mv);
	if (ret < 0) {
		pr_err("get vindpm fail, ret:%d\n", ret);
		return ret;
	}
	*uv = mv * 1000;
...
static struct charger_ops upm6922_chg_ops = {
   ...
   /* Event */
    ...
    /*A06_V code for SR-AL7160V-01-164 by yexuedong at 20240910 end*/
    /*HS07 code for HS07-752 by xiongxiaoliang at 20250415 start*/
    .enable_hz = upm6922_enable_hz,
    /*HS07 code for HS07-752 by xiongxiaoliang at 20250415 end*/
    /* HS07 code for HS07-1480 by lina at 20250425 start */
    #if IS_ENABLED(CONFIG_CUSTOM_USBIF)
    .set_vindpm = upm6922_set_vindpm,
    #endif//CONFIG_CUSTOM_USBIF
    /* HS07 code for HS07-1480 by lina at 20250425 end */
};
...
static int upm6922_enable_vbus(struct regulator_dev *rdev)
{
    ...
    /*HS07 code for HS07-46 by lina at 20250228 end*/
    #endif
    /*HS07 code for HS07-181 by xiongxiaoliang at 20250318 start*/
    /* HST11 code for AX7800A-874 by wenyaqi at 20250522 start */
    ret = upm6922_get_hiz_mode(upm, &hiz_enable);
    if (ret >= 0 && hiz_enable == true) {
    /* HST11 code for AX7800A-874 by wenyaqi at 20250522 end */
        pr_err("use otg func in hiz mode, need exit hiz\n");
        upm6922_disable_hiz_mode(upm);
    }
    /*HS07 code for HS07-181 by xiongxiaoliang at 20250318 end*/

    /* HST11 code for AX7800A-2090 by zhangziyi at 20250610 start */
    #if defined(CONFIG_CUSTOM_PROJECT_HST11)
    upm6922_update_bits(upm, UPM6922_REG_06, REG06_BOOSTV_MASK,
                            REG06_BOOSTV_5P3V << REG06_BOOSTV_SHIFT);
    pr_err("boost to 5.3V\n");
    #endif //CONFIG_CUSTOM_PROJECT_HST11
    /* HST11 code for AX7800A-2090 by zhangziyi at 20250610 end */
    ret = upm6922_enable_otg(upm);
    pr_err("ret = %d\n", ret);
...
static int upm6922_disable_vbus(struct regulator_dev *rdev)
{
    ...
    #if defined(CONFIG_CUSTOM_PROJECT_HST11)
    upm6922_update_bits(upm, UPM6922_REG_06, REG06_BOOSTV_MASK,
                            REG06_BOOSTV_5V << REG06_BOOSTV_SHIFT);
    pr_err("boost to 5V\n");
    #endif //CONFIG_CUSTOM_PROJECT_HST11
    /* HST11 code for AX7800A-2090 by zhangziyi at 20250610 end */
...
static int upm6922_charger_probe(struct i2c_client *client)
{
    ...
    /*A06_V code for SR-AL7160V-01-206  by yexuedong at 20240902 start*/
    gxy_bat_set_chginfo(GXY_BAT_CHG_INFO_UPM6922P);
    /*A06_V code for SR-AL7160V-01-206  by yexuedong at 20240902 end*/
    /*A06_V code for SR-AL7160V-01-164 by yexuedong at 20240910 start*/
    ...
    /*HS07 code for HS07-752 by xiongxiaoliang at 20250415 start*/
    #if IS_ENABLED(CONFIG_CUSTOM_USBIF)
    INIT_DELAYED_WORK(&upm->set_input_volt_lim_work, upm6922_delay_set_input_volt_lim);
    #endif
    /*HS07 code for HS07-752 by xiongxiaoliang at 20250415 end*/
...
static void upm6922_charger_shutdown(struct i2c_client *client)
{
    ...
    /*HS07 code for HS07-1348 by yexuedong at 20250423 start*/
    upm6922_set_chargecurrent(upm, 240);
    /*HS07 code for HS07-1348 by yexuedong at 20250423 end*/
