...
/*HS07 W code for AL7761C-1 by xiaozhenting at 20250806 start*/
#include "../../misc/mediatek/typec/tcpc/inc/tcpm.h"
/*HS07 W code for AL7761C-1 by xiaozhenting at 20250806 end*/
#include "gxy_battery_ttf.h"

/* A07 LTE_BOS code for AL7761C-1 by xiaojian at 20250813 start */
int gxy_get_pps_apdo(struct mtk_charger *info);
/* A07 LTE_BOS code for AL7761C-1 by xiaojian at 20250813 end */
        ...
        case TCP_NOTIFY_PD_STATE:
            ...
            if (battery->chg_info != NULL) {
                ...
                if (battery->pd_type == PD_CONNECT_PE_READY_SNK_APDO) {
                    gxy_get_pps_apdo(battery->chg_info);
                }
                #endif
...
/*Tab A9 code for AX6739W-26 by xiaojian at 20250628 start*/
        battery->psy_chg = power_supply_get_by_name("primary_chg");
/*Tab A9 code for AX6739W-26 by xiaojian at 20250628 end*/
        if (IS_ERR_OR_NULL(battery->psy_chg)) {
...
static void gxy_bat_calc_time_to_full(struct gxy_battery_ttf *battery)
{
    ...
    if (delayed_work_pending(&battery->ttf_d->timetofull_work)) {
        ...
        if ((info->enable_hv_charging == true) && (info->afc_result == 1 ||
            ...
            /*HS07 W code for AL7761C-1 by xiaozhenting at 20250806 end*/
		/*HS07 code for P250509-02972 by yexuedong at 20250512 start*/
                #if defined(CONFIG_CUSTOM_PROJECT_HS07)
                if (battery->pd_type == PD_CONNECT_PE_READY_SNK_APDO) {
                    /*HS07 code for HS07-4806 by xiongxiaoliang at 20250630 start*/
                    if ((info->pwr_max != 0) && (info->pwr_max < GXY_PD_25W_CHARGING_POWER)) {
                        charge = battery->ttf_d->ttf_pd20_charge_current;
                    } else {
                ...
                #endif
                /*HS07 code for P250509-02972 by yexuedong at 20250512 end*/
                /*HS07 code for HS07-3358 by lina at 20250522 start*/
                if (battery->usb_vol_now < USB_TYPE_CTOC_VBUS && battery->pd_type == PD_CONNECT_PE_READY_SNK_PD30) {
                    charge = USB_TYPE_CTOC_DEFAULT_CURR;
                }
                /*HS07 code for HS07-3358 by lina at 20250522 end*/
            ...
            /*HS07 code for P250509-02972 by yexuedong at 20250512 strat*/
        ...
        /*HS07 code for P250509-02972 by yexuedong at 20250512 start*/
        GXY_PSY_ERR("%s: T:%d sec = [%dh%dmin], current:%d, enable_hv_charging:%d\n", __func__, battery->ttf_d->timetofull,
            h_time, min_time, charge, info->enable_hv_charging);
        /*HS07 code for P250509-02972 by yexuedong at 20250512 end*/
    } else {
        battery->ttf_d->timetofull = -1;
    }
...
static int gxy_ttf_parse_dt(struct gxy_battery_ttf *battery)
{
    ...
    /*HS07 code for HS07-4806 by xiongxiaoliang at 20250630 start*/
    ret = of_property_read_u32(np, "battery,ttf_pd20_charge_current",
                    &pttfdata->ttf_pd20_charge_current);
    if (ret) {
        pttfdata->ttf_pd20_charge_current = PD20_CHARGE_DEFAULT_CURR;
        GXY_PSY_ERR("%s: ttf_pd20_charge_current is Empty, Default value %d\n",
            __func__, pttfdata->ttf_pd20_charge_current);
    }
    /*HS07 code for HS07-4806 by xiongxiaoliang at 20250630 end*/
