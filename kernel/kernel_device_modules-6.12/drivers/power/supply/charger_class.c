...
int charger_dev_set_vindpm(struct charger_device *chg_dev, bool en)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->set_vindpm)
		return chg_dev->ops->set_vindpm(chg_dev, en);
	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_set_vindpm);
