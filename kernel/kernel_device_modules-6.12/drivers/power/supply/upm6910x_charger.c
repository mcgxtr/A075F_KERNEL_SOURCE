                ...
                struct upm6910x_state *state, bool type_recheck)
{
    ...
    /*Tab A9 code for SR-AX6739A-01-494 by qiaodan at 20230506 end*/
    /*Tab A9 code for SR-AX6739A-01-500 by wenyaqi at 20230515 start*/
    /*Tab A9 code for AX6739W-47 by wangmingye at 20250923 start*/
    if (upm->psy_type == POWER_SUPPLY_TYPE_USB_DCP && upm->hvdcp_done != true) {
    /*Tab A9 code for AX6739W-47 by wangmingye at 20250923 end*/
...
static void charger_detect_work_func(struct work_struct *work)
{
    ...
    /*Tab A9 code for AX6739A-2299 by hualei at 20230713 start*/
	pr_err("%s online_pre = %d,state.online = %d\n", __func__,
           online_pre, state.online);
    ...
    /*Tab A9 code for AX6739A-2344 by hualei at 20230717 start*/
	pr_err("%s upm->force_dpdm = %d\n", __func__, upm->force_dpdm);
            ...
            case 2:
                *chr_type = POWER_SUPPLY_CHARGE_TYPE_FAST;
                ...
                if (batt_psy == NULL || IS_ERR(batt_psy)) {
                    ...
                    if (vbat > 4350000) {
                        /*Tab A9_NA code for AX6739W-26 by xiaozhenting at 20250902 start*/
                        *chr_type = POWER_SUPPLY_CHARGE_TYPE_TAPER_EXT;
                        last_chr_type = POWER_SUPPLY_CHARGE_TYPE_TAPER_EXT;
                        /*Tab A9_NA code for AX6739W-26 by xiaozhenting at 20250902 end*/
                        pr_err("%s: vbat: > 4.35v\n", __func__);
                        return 0;
                    }
                }
