...
void battery_update(struct mtk_battery_manager *bm)
{
	...
	if (bat_data->bat_capacity == 100 &&
		...
		pr_err("battery_update set FULL! ui:%d status:%d\n",
			bat_data->bat_capacity, bat_data->bat_status);
	}
	/*HS07 code for P250509-06802 by lina at 20250515 start*/
	/* HST11 code for P250701-05046 by zhangziyi at 20250703 start */
	#if !defined(CONFIG_ODM_CUSTOM_FACTORY_BUILD)
	else if (bat_data->bat_status == POWER_SUPPLY_STATUS_NOT_CHARGING &&
		g_batt_soc_rechg == true) {
		bat_data->bat_status = POWER_SUPPLY_STATUS_FULL;
		pr_info("In battery protect and battery_update set FULL! ui:%d status:%d\n",
			bat_data->bat_capacity, bat_data->bat_status);
	}
	#endif//CONFIG_ODM_CUSTOM_FACTORY_BUILD
	/* HST11 code for P250701-05046 by zhangziyi at 20250703 end */
	/*HS07 code for P250509-06802 by lina at 20250515 end*/
	pr_info("battery_update status: ui:%d status:%d\n",
